...
public class CompactingMemStore extends AbstractMemStore {
    ...
    public static final long DEEP_OVERHEAD = ClassSize.align(AbstractMemStore.DEEP_OVERHEAD + // Store, CompactionPipeline,
    6 * ClassSize.REFERENCE + // MemStoreCompactor, inMemoryFlushInProgress, allowCompaction,
    // indexType
    Bytes.// inmemoryFlushSize
    SIZEOF_LONG + // compositeSnapshot and inWalReplay
    2 * Bytes.SIZEOF_BOOLEAN + // inMemoryFlushInProgress and allowCompaction
    2 * ClassSize.ATOMIC_BOOLEAN + CompactionPipeline.DEEP_OVERHEAD + MemStoreCompactor.DEEP_OVERHEAD);
    ...
    private void initInmemoryFlushSize(Configuration conf) {
        ...
        factor = conf.getDouble(IN_MEMORY_FLUSH_THRESHOLD_FACTOR_KEY, IN_MEMORY_FLUSH_THRESHOLD_FACTOR_DEFAULT);
    ...
    public MemStoreSize getFlushableSize() {
        ...
        org.zlab.ocov.tracker.Runtime.update(mss, 76);
    ...
    void flushInMemory() throws IOException {
        ...
        try {
            ...
            try {
                ...
                LOG.warn("Unable to run in-memory compaction on {}/{}; exception={}", getRegionServices().getRegionInfo().getEncodedName(), getFamilyName(), e);
            }
        } finally {
    ...
    protected boolean shouldFlushInMemory() {
        if (this.active.getDataSize() > inmemoryFlushSize) {
    ...
    private void pushTailToSnapshot() {
        ...
        org.zlab.ocov.tracker.Runtime.update(segments, 77);
    ...
    private void pushPipelineToSnapshot() {
        ...
        while (!done) {
            ...
            org.zlab.ocov.tracker.Runtime.update(segments, 78);
            pushToSnapshot(segments.getStoreSegments());
