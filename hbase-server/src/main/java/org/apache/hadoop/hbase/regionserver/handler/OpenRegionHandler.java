...
public class OpenRegionHandler extends EventHandler {
    ...
    public OpenRegionHandler(final Server server, final RegionServerServices rsServices, RegionInfo regionInfo, TableDescriptor htd, long masterSystemTime) {
        this(server, rsServices, regionInfo, htd, masterSystemTime, EventType.M_RS_OPEN_REGION);
    }

    protected OpenRegionHandler(final Server server, final RegionServerServices rsServices, final RegionInfo regionInfo, final TableDescriptor htd, long masterSystemTime, EventType eventType) {
    ...
    private void doCleanUpOnFailedOpen(HRegion region) throws IOException {
        try {
            ...
            rsServices.reportRegionStateTransition(new RegionStateTransitionContext(TransitionCode.FAILED_OPEN, HConstants.NO_SEQNUM, Procedure.NO_PROC_ID, -1, regionInfo));
        }
    }
    ...
    static class PostOpenDeployTasksThread extends Thread {
        ...
        PostOpenDeployTasksThread(final HRegion region, final Server server, final RegionServerServices services, final AtomicBoolean signaller, long masterSystemTime) {
        ...
        public void run() {
            try {
                org.zlab.ocov.tracker.Runtime.update(this, 883);
                this.services.postOpenDeployTasks(new PostOpenDeployContext(region, Procedure.NO_PROC_ID, masterSystemTime));
            } catch (Throwable e) {
