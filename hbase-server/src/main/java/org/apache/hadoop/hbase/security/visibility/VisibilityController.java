...
public class // TODO: break out Observer functions into separate class/sub-class.
VisibilityController implements MasterCoprocessor, RegionCoprocessor, VisibilityLabelsService.Interface, MasterObserver, RegionObserver {
    ...
    @Override
    public void postSetSplitOrMergeEnabled(final ObserverContext<MasterCoprocessorEnvironment> ctx, final boolean newValue, final MasterSwitchType switchType) throws IOException {
    }
    ...
    public void preBatchMutate(ObserverContext<RegionCoprocessorEnvironment> c, MiniBatchOperationInProgress<Mutation> miniBatchOp) throws IOException {
        ...
        for (int i = 0; i < miniBatchOp.size(); i++) {
            ...
            for (CellScanner cellScanner = m.cellScanner(); ((org.apache.hadoop.hbase.CellScanner) org.zlab.ocov.tracker.Runtime.update(cellScanner, 164, c, miniBatchOp)).advance(); ) {
    ...
    public void prePrepareTimeStampForDeleteVersion(ObserverContext<RegionCoprocessorEnvironment> ctx, Mutation delete, Cell cell, byte[] byteNow, Get get) throws IOException {
        ...
        org.zlab.ocov.tracker.Runtime.update(get, 165, ctx, delete, cell, byteNow, get);
    ...
    public void preScannerOpen(ObserverContext<RegionCoprocessorEnvironment> e, Scan scan) throws IOException {
        ...
        if (visibilityLabelFilter != null) {
            ...
            if (filter != null) {
                scan.setFilter(((FilterList) org.zlab.ocov.tracker.Runtime.update(new FilterList(filter, visibilityLabelFilter), 166, e, scan)));
            } else {
    ...
    public void preGetOp(ObserverContext<RegionCoprocessorEnvironment> e, Get get, List<Cell> results) throws IOException {
        ...
        if (visibilityLabelFilter != null) {
            ...
            if (filter != null) {
                get.setFilter(((FilterList) org.zlab.ocov.tracker.Runtime.update(new FilterList(filter, visibilityLabelFilter), 167, e, get, results)));
            } else {
    ...
    public Cell postMutationBeforeWAL(ObserverContext<RegionCoprocessorEnvironment> ctx, MutationType opType, Mutation mutation, Cell oldCell, Cell newCell) throws IOException {
        ...
        while (tagsItr.hasNext()) {
            ...
            if (tag.getType() != TagType.VISIBILITY_TAG_TYPE && tag.getType() != TagType.VISIBILITY_EXP_SERIALIZATION_FORMAT_TAG_TYPE) {
                tags.add(tag);
                org.zlab.ocov.tracker.Runtime.update(tags, 168, ctx, opType, mutation, oldCell, newCell);
            }
        }
