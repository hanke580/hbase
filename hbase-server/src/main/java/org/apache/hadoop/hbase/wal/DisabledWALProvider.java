...
class DisabledWALProvider implements WALProvider {
    ...
    private static class DisabledWAL implements WAL {
        ...
        private long append(RegionInfo info, WALKeyImpl key, WALEdit edits, boolean inMemstore) throws IOException {
            ...
            org.zlab.ocov.tracker.Runtime.update(writeEntry, 215, info, key, edits, inMemstore);
            if (!edits.isReplay()) {
        ...
        @Override
        public Long startCacheFlush(final byte[] encodedRegionName, Map<byte[], Long> flushedFamilyNamesToSeq) {
