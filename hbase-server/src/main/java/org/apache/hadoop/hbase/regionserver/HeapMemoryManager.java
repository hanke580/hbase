...
public class HeapMemoryManager {
    ...
    private static final int CLUSTER_MINIMUM_MEMORY_THRESHOLD = (int) (CONVERT_TO_PERCENTAGE * HConstants.HBASE_CLUSTER_MINIMUM_MEMORY_THRESHOLD);
    ...
    public static HeapMemoryManager create(Configuration conf, FlushRequester memStoreFlusher, Server server, RegionServerAccounting regionServerAccounting) {
        ...
        if (lruCache != null) {
            return ((HeapMemoryManager) org.zlab.ocov.tracker.Runtime.update(new HeapMemoryManager(lruCache, memStoreFlusher, server, regionServerAccounting), 79, conf, memStoreFlusher, server, regionServerAccounting));
        }
        return null;
    }
    ...
    private boolean doInit(Configuration conf) {
        ...
        if (globalMemStorePercent < globalMemStorePercentMinRange) {
            LOG.warn("Setting " + MEMSTORE_SIZE_MIN_RANGE_KEY + " to " + globalMemStorePercent + ", same value as " + MemorySizeUtil.MEMSTORE_SIZE_KEY + " because supplied value greater than initial memstore size value.");
        ...
        if (globalMemStorePercent > globalMemStorePercentMaxRange) {
            LOG.warn("Setting " + MEMSTORE_SIZE_MAX_RANGE_KEY + " to " + globalMemStorePercent + ", same value as " + MemorySizeUtil.MEMSTORE_SIZE_KEY + " because supplied value less than initial memstore size value.");
    ...
    private class HeapMemoryTunerChore extends ScheduledChore implements FlushRequestListener {
        ...
        private void tune() {
            ...
            if (result != null && result.needsTuning()) {
                ...
                if (CONVERT_TO_PERCENTAGE - (gml + bcul) < CLUSTER_MINIMUM_MEMORY_THRESHOLD) {
                    LOG.info("Current heap configuration from HeapMemoryTuner exceeds " + "the threshold required for successful cluster operation. " + "The combined value cannot exceed 0.8. " + MemorySizeUtil.MEMSTORE_SIZE_KEY + " is " + memstoreSize + " and " + HFILE_BLOCK_CACHE_SIZE_KEY + " is " + blockCacheSize);
