...
public class CompactionPipeline {
    ...
    public final static long FIXED_OVERHEAD = ClassSize.align(ClassSize.OBJECT + (3 * ClassSize.REFERENCE) + Bytes.SIZEOF_LONG);
    ...
    public boolean pushHead(MutableSegment segment) {
        ...
        if (region != null) {
            region.addMemStoreSize(memstoreAccounting.getDataSize(), memstoreAccounting.getHeapSize(), memstoreAccounting.getOffHeapSize(), memstoreAccounting.getCellsCount());
        }
     ...
     */
    @edu.umd.cs.findbugs.annotations.SuppressWarnings(value = "VO_VOLATILE_INCREMENT", justification = "Increment is done under a synchronize block so safe")
    public boolean swap(VersionedSegmentsList versionedList, ImmutableSegment segment, boolean closeSuffix, boolean updateRegionSize) {
        ...
        if (updateRegionSize && region != null) {
            ...
            LOG.debug("Suffix data size={}, new segment data size={}, " + "suffix heap size={}, new segment heap size={}" + "suffix off heap size={}, new segment off heap size={}" + "suffix cells count={}, new cells count={}", suffixDataSize, newDataSize, suffixHeapSize, newHeapSize, suffixOffHeapSize, newOffHeapSize, suffixCellsCount, newCellsCount);
        }
        return true;
    }
            ...
            for (ImmutableSegment s : pipeline) {
                if (s.canBeFlattened()) {
                    ...
                    if (region != null) {
                        ...
                        org.zlab.ocov.tracker.Runtime.update(mss, 88, requesterVersion, idxType, action);
