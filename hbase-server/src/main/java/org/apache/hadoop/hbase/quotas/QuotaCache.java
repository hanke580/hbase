...
public class QuotaCache implements Stoppable {
    ...
    // 5min
    private static final int REFRESH_DEFAULT_PERIOD = 5 * 60000;

    // N * REFRESH_DEFAULT_PERIOD
    ...
    public UserQuotaState getUserQuotaState(final UserGroupInformation ugi) {
        return computeIfAbsent(userQuotaCache, ugi.getShortUserName(), UserQuotaState::new, this::triggerCacheRefresh);
    }
    ...
    private class QuotaRefresherChore extends ScheduledChore {
        ...
        @Override
        @edu.umd.cs.findbugs.annotations.SuppressWarnings(value = "GC_UNRELATED_TYPES", justification = "I do not understand why the complaints, it looks good to me -- FIX")
        ...
        private <K, V extends QuotaState> void fetch(final String type, final ConcurrentHashMap<K, V> quotasMap, final Fetcher<K, V> fetcher) {
