...
public final class SnapshotDescriptionUtils {
    ...
    public static Path getWorkingSnapshotDir(final Path rootDir, final Configuration conf) {
        return new Path(conf.get(SNAPSHOT_WORKING_DIR, getDefaultWorkingSnapshotDir(rootDir).toString()));
    }
     ...
     */
    public static Path getWorkingSnapshotDir(SnapshotDescription snapshot, final Path rootDir, Configuration conf) {
        return getWorkingSnapshotDir(((org.apache.hadoop.hbase.shaded.protobuf.generated.SnapshotProtos.SnapshotDescription) org.zlab.ocov.tracker.Runtime.update(snapshot, 191, snapshot, rootDir, conf)).getName(), rootDir, conf);
    }
     ...
     */
    public static boolean isWithinDefaultWorkingDir(final Path workingDir, Configuration conf) throws IOException {
    ...
    public static SnapshotDescription validate(SnapshotDescription snapshot, Configuration conf) throws IllegalArgumentException, IOException {
        ...
        if (ttl == SnapshotDescriptionUtils.NO_SNAPSHOT_TTL_SPECIFIED || ttl > TimeUnit.MILLISECONDS.toSeconds(Long.MAX_VALUE)) {
            final long defaultSnapshotTtl = conf.getLong(HConstants.DEFAULT_SNAPSHOT_TTL_CONFIG_KEY, HConstants.DEFAULT_SNAPSHOT_TTL);
            if (LOG.isDebugEnabled()) {
                LOG.debug("Snapshot current TTL value: {} resetting it to default value: {}", ttl, defaultSnapshotTtl);
            }
    ...
    public static void writeSnapshotInfo(SnapshotDescription snapshot, Path workingDir, FileSystem fs) throws IOException {
        FsPermission perms = CommonFSUtils.getFilePermissions(fs, fs.getConf(), HConstants.DATA_FILE_UMASK_KEY);
    ...
    static boolean shouldSkipRenameSnapshotDirectories(URI workingURI, URI rootURI) {
        ...
        if (workingURI.getUserInfo() != null && !workingURI.getUserInfo().equals(rootURI.getUserInfo())) {
            return true;
        }
        return false;
    }
     ...
     */
    public static boolean isSnapshotOwner(org.apache.hadoop.hbase.client.SnapshotDescription snapshot, User user) {
        if (user == null)
            return false;
    ...
    private static SnapshotDescription writeAclToSnapshotDescription(SnapshotDescription snapshot, Configuration conf) throws IOException {
        ListMultimap<String, UserPermission> perms = User.runAsLoginUser(new PrivilegedExceptionAction<ListMultimap<String, UserPermission>>() {

            @Override
            public ListMultimap<String, UserPermission> run() throws Exception {
                return PermissionStorage.getTablePermissions(conf, TableName.valueOf(snapshot.getTable()));
            }
        });
    ...
    public static boolean isExpiredSnapshot(long snapshotTtl, long snapshotCreatedTime, long currentTime) {
        return snapshotCreatedTime > 0 && snapshotTtl > HConstants.DEFAULT_SNAPSHOT_TTL && snapshotTtl < TimeUnit.MILLISECONDS.toSeconds(Long.MAX_VALUE) && (snapshotCreatedTime + TimeUnit.SECONDS.toMillis(snapshotTtl)) < currentTime;
    }
}
