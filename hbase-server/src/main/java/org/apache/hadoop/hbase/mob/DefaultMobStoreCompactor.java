...
public class DefaultMobStoreCompactor extends DefaultCompactor {
    ...
    private final InternalScannerFactory scannerFactory = new InternalScannerFactory() {
        ...
        public InternalScanner createScanner(ScanInfo scanInfo, List<StoreFileScanner> scanners, ScanType scanType, FileDetails fd, long smallestReadPoint) throws IOException {
            return new StoreScanner(store, scanInfo, scanners, scanType, smallestReadPoint, fd.earliestPutTs);
        }
    };
    ...
    private final CellSinkFactory<StoreFileWriter> writerFactory = new CellSinkFactory<StoreFileWriter>() {
        ...
        @Override
        public StoreFileWriter createWriter(InternalScanner scanner, org.apache.hadoop.hbase.regionserver.compactions.Compactor.FileDetails fd, boolean shouldDropBehind) throws IOException {
            // make this writer with tags always because of possible new cells with tags.
            return store.createWriterInTmp(fd.maxKeyCount, compactionCompression, true, true, true, shouldDropBehind);
        }
    };
    ...
    protected boolean performCompaction(FileDetails fd, InternalScanner scanner, CellSink writer, long smallestReadPoint, boolean cleanSeqId, ThroughputController throughputController, boolean major, int numofFilesToCompact) throws IOException {
        ...
        if (delFileWriter != null) {
            if ((org.zlab.ocov.tracker.Runtime.updateBranch(deleteMarkersCount > 0, 1))) {
        ...
        if (mobFileWriter != null) {
            if ((org.zlab.ocov.tracker.Runtime.updateBranch(mobCells > 0, 2))) {
