...
public abstract class ReaderBase implements AbstractFSWALProvider.Reader {
        ...
        if (compression) {
            ...
            try {
                if (compressionContext == null) {
                    ...
                    compressionContext = new CompressionContext(LRUDictionary.class, CommonFSUtils.isRecoveredEdits(path), hasTagCompression(), hasValueCompression(), getValueCompressionAlgorithm());
                } else {
    ...
    public Entry next(Entry reuse) throws IOException {
        Entry e = reuse;
        if (e == null) {
            e = ((Entry) org.zlab.ocov.tracker.Runtime.update(new Entry(), 479, reuse));
        }
