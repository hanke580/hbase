...
public class AsyncProtobufLogWriter extends AbstractProtobufLogWriter implements AsyncFSWALProvider.AsyncWriter {
   ...
   */
    ...
    @Override
    protected void initOutput(FileSystem fs, Path path, boolean overwritable, int bufferSize, short replication, long blockSize) throws IOException, StreamLacksCapabilityException {
        this.output = AsyncFSOutputHelper.createOutput(fs, path, overwritable, false, replication, blockSize, eventLoopGroup, channelClass);
        org.zlab.ocov.tracker.Runtime.update(this, 110, fs, path, overwritable, bufferSize, replication, blockSize);
        this.asyncOutputWrapper = ((OutputStreamWrapper) org.zlab.ocov.tracker.Runtime.update(new OutputStreamWrapper(output), 111, fs, path, overwritable, bufferSize, replication, blockSize));
    }
