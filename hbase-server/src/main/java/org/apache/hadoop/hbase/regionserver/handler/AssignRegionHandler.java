...
public class AssignRegionHandler extends EventHandler {
    ...
    public void process() throws IOException {
        ...
        rs.postOpenDeployTasks(((PostOpenDeployContext) org.zlab.ocov.tracker.Runtime.update(new PostOpenDeployContext(region, openProcId, masterSystemTime), 853)));
        rs.addRegion(region);
    ...
    protected void handleException(Throwable t) {
        LOG.warn("Fatal error occurred while opening region {}, aborting...", regionInfo.getRegionNameAsString(), t);
        ...
        getServer().abort("Failed to open region " + regionInfo.getRegionNameAsString() + " and can not recover", t);
    }

    public static AssignRegionHandler create(HRegionServer server, RegionInfo regionInfo, long openProcId, TableDescriptor tableDesc, long masterSystemTime) {
        EventType eventType;
        if (regionInfo.isMetaRegion()) {
            eventType = EventType.M_RS_OPEN_META;
        } else if (regionInfo.getTable().isSystemTable() || (tableDesc != null && tableDesc.getPriority() >= HConstants.ADMIN_QOS)) {
            eventType = EventType.M_RS_OPEN_PRIORITY_REGION;
        } else {
            eventType = EventType.M_RS_OPEN_REGION;
        }
        return new AssignRegionHandler(server, regionInfo, openProcId, tableDesc, masterSystemTime, eventType);
    }
}
