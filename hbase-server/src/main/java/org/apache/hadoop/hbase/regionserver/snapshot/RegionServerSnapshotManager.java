...
public class RegionServerSnapshotManager extends RegionServerProcedureManager {
    ...
    public Subprocedure buildSubprocedure(SnapshotDescription snapshot) {
        ...
        org.zlab.ocov.tracker.Runtime.update(snapshot, 577, snapshot);
    ...
    private List<HRegion> getRegionsToSnapshot(SnapshotDescription snapshot) throws IOException {
        List<HRegion> onlineRegions = (List<HRegion>) rss.getRegions(TableName.valueOf(snapshot.getTable()));
    ...
    static class SnapshotSubprocedurePool {
        ...
        SnapshotSubprocedurePool(String name, Configuration conf, Abortable abortable) {
            ...
            this.name = name;
            executor = Threads.getBoundedCachedThreadPool(threads, keepAlive, TimeUnit.MILLISECONDS, new ThreadFactoryBuilder().setNameFormat("rs(" + name + ")-snapshot-pool-%d").setDaemon(true).setUncaughtExceptionHandler(Threads.LOGGING_EXCEPTION_HANDLER).build());
