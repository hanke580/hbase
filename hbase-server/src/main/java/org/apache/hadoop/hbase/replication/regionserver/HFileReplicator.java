...
public class HFileReplicator implements Closeable {
    ...
    public HFileReplicator(Configuration sourceClusterConf, String sourceBaseNamespaceDirPath, String sourceHFileArchiveDirPath, Map<String, List<Pair<byte[], List<String>>>> tableQueueMap, Configuration conf, Connection connection, List<String> sourceClusterIds) throws IOException {
        ...
        this.hbaseStagingDir = new Path(CommonFSUtils.getRootDir(conf), HConstants.BULKLOAD_STAGING_DIR_NAME);
        ...
        this.exec = Threads.getBoundedCachedThreadPool(maxCopyThreads, 60, TimeUnit.SECONDS, new ThreadFactoryBuilder().setDaemon(true).setNameFormat("HFileReplicationCopier-%1$d-" + this.sourceBaseNamespaceDirPath).build());
    ...
    public Void replicate() throws IOException {
        ...
        for (Entry<String, Path> tableStagingDir : tableStagingDirsMap.entrySet()) {
            ...
            org.zlab.ocov.tracker.Runtime.update(tableName, 828);
    ...
    private void doBulkLoad(LoadIncrementalHFiles loadHFiles, Table table, Deque<LoadQueueItem> queue, RegionLocator locator, int maxRetries) throws IOException {
        int count = 0;
    ...
    private class Copier implements Callable<Void> {
        ...
        public Copier(FileSystem sourceFs, final Path stagingDir, final List<String> hfiles) throws IOException {
            ...
            for (int i = 0; i < totalHFiles; i++) {
                ...
                try {
                    ...
                    try {
                        ...
                        LOG.debug("Failed to copy hfile from " + sourceHFilePath + " to " + localHFilePath + ". Hence ignoring this hfile from replication..", e1);
                        continue;
                    }
                }
