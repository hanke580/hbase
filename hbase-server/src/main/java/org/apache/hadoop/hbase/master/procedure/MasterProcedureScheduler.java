...
public class MasterProcedureScheduler extends AbstractProcedureScheduler {
    ...
    private static final AvlKeyComparator<ServerQueue> SERVER_QUEUE_KEY_COMPARATOR = (n, k) -> n.compareKey((ServerName) k);

    private final static AvlKeyComparator<TableQueue> TABLE_QUEUE_KEY_COMPARATOR = (n, k) -> n.compareKey((TableName) k);

    private final static AvlKeyComparator<PeerQueue> PEER_QUEUE_KEY_COMPARATOR = (n, k) -> n.compareKey((String) k);

    private final static AvlKeyComparator<MetaQueue> META_QUEUE_KEY_COMPARATOR = (n, k) -> n.compareKey((TableName) k);
    ...
    protected boolean queueHasRunnables() {
        return metaRunQueue.hasRunnables() || tableRunQueue.hasRunnables() || serverRunQueue.hasRunnables() || peerRunQueue.hasRunnables();
    }
    ...
    private <T extends Comparable<T>, TNode extends Queue<T>> void clear(TNode treeMap, FairQueue<T> fairq, AvlKeyComparator<TNode> comparator) {
        while (treeMap != null) {
    ...
    private void removeServerQueue(ServerName serverName) {
        ...
        serverBuckets[index] = AvlTree.remove(serverBuckets[index], serverName, SERVER_QUEUE_KEY_COMPARATOR);
        ...
        try {
            ...
            for (int i = 0; i < regionInfos.length; ++i) {
                ...
                if (!regionLocks[i].tryExclusiveLock(procedure)) {
                    LOG.info("Waiting on xlock for {} held by pid={}", procedure, regionLocks[i].getExclusiveLockProcIdOwner());
    ...
    public String toString() {
        ToStringBuilder builder = new ToStringBuilder(this, ToStringStyle.SHORT_PREFIX_STYLE).appendSuper(super.toString());
        schedLock();
        try {
            for (int i = 0; i < serverBuckets.length; i++) {
