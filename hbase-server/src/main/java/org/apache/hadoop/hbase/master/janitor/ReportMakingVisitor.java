...
class ReportMakingVisitor implements MetaTableAccessor.CloseableVisitor {
    ...
    private RegionInfo metaTableConsistencyCheck(Result metaTableRow) {
        ...
        if (!Bytes.equals(metaTableRow.getRow(), ri.getRegionName())) {
            LOG.warn("INCONSISTENCY: Row name is not equal to serialized info:regioninfo content; " + "row={} {}; See if RegionInfo is referenced in another hbase:meta row? Delete?", Bytes.toStringBinary(metaTableRow.getRow()), ri.getRegionNameAsString());
            return null;
        }
