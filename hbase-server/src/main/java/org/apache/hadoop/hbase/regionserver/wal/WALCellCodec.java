...
public class WALCellCodec implements Codec {
    ...
    static class BaosAndCompressor extends ByteArrayOutputStream implements ByteStringCompressor {
        ...
        public ByteString compress(byte[] data, Enum dictIndex) throws IOException {
            ...
            // Only resets the count - we reuse the byte array.
            reset();
            return result;
        }
    ...
    static class CompressedKvEncoder extends BaseEncoder {
        ...
        public void write(Cell cell) throws IOException {
            ...
            PrivateCellUtil.compressRow(out, cell, compression.getDictionary(CompressionContext.DictionaryIndex.ROW));
            PrivateCellUtil.compressFamily(out, cell, compression.getDictionary(CompressionContext.DictionaryIndex.FAMILY));
            PrivateCellUtil.compressQualifier(out, cell, compression.getDictionary(CompressionContext.DictionaryIndex.QUALIFIER));
    ...
    static class CompressedKvDecoder extends BaseDecoder {
        ...
        protected Cell parseCell() throws IOException {
            ...
            // the row
            int elemLen = readIntoArray(backingArray, pos + Bytes.SIZEOF_SHORT, compression.getDictionary(CompressionContext.DictionaryIndex.ROW));
            checkLength(elemLen, Short.MAX_VALUE);
            pos = Bytes.putShort(backingArray, pos, (short) elemLen);
            pos += elemLen;
            // family
            elemLen = readIntoArray(backingArray, pos + Bytes.SIZEOF_BYTE, compression.getDictionary(CompressionContext.DictionaryIndex.FAMILY));
            checkLength(elemLen, Byte.MAX_VALUE);
            pos = Bytes.putByte(backingArray, pos, (byte) elemLen);
            pos += elemLen;
            // qualifier
            elemLen = readIntoArray(backingArray, pos, compression.getDictionary(CompressionContext.DictionaryIndex.QUALIFIER));
            pos += elemLen;
            ...
            return ((KeyValue) org.zlab.ocov.tracker.Runtime.update(new KeyValue(backingArray, 0, length), 114));
        }
    ...
    public Decoder getDecoder(InputStream is) {
        return (compression == null) ? new KeyValueCodecWithTags.KeyValueDecoder(is) : new CompressedKvDecoder(is, compression);
    }
    ...
    public Encoder getEncoder(OutputStream os) {
        os = (os instanceof ByteBufferWriter) ? os : ((ByteBufferWriterOutputStream) org.zlab.ocov.tracker.Runtime.update(new ByteBufferWriterOutputStream(os), 113, os));
        if (compression == null) {
