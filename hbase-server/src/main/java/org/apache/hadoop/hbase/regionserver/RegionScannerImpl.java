...
class RegionScannerImpl implements RegionScanner, Shipper, RpcCallback {
    ...
    private static boolean hasNonce(HRegion region, long nonce) {
        ...
        return nonce != HConstants.NO_NONCE && rsServices != null && rsServices.getNonceManager() != null;
    }
    ...
    public boolean nextRaw(List<Cell> outResults, ScannerContext scannerContext) throws IOException {
        ...
        if (outResults.isEmpty()) {
            ...
            org.zlab.ocov.tracker.Runtime.update(outResults, 46, outResults, scannerContext);
        }
            ...
            if (joinedContinuationRow == null) {
                ...
                if (shouldStop) {
                    if (hasFilterRow) {
                        ...
                        org.zlab.ocov.tracker.Runtime.update(results, 47, results, scannerContext);
                    }
                ...
                if (filterRowKey(current)) {
                    ...
                    results.clear();
                    org.zlab.ocov.tracker.Runtime.update(results, 52, results, scannerContext);
                ...
                if (hasFilterRow) {
                    ...
                    org.zlab.ocov.tracker.Runtime.update(results, 49, results, scannerContext);
                ...
                if (isEmptyRow || ret == FilterWrapper.FilterRowRetCode.EXCLUDE || filterRow()) {
                    incrementCountOfRowsFilteredMetric(scannerContext);
                    results.clear();
                    org.zlab.ocov.tracker.Runtime.update(results, 51, results, scannerContext);
                    boolean moreRows = nextRow(scannerContext, current);
                    if (!moreRows) {
                        return scannerContext.setScannerState(NextState.NO_MORE_VALUES).hasMoreValues();
                    }
    ...
    @edu.umd.cs.findbugs.annotations.SuppressWarnings(value = "IS2_INCONSISTENT_SYNC", justification = "this method is only called inside close which is synchronized")
