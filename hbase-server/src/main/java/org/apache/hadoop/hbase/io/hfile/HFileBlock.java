...
public class HFileBlock implements Cacheable {
    ...
    static final CacheableDeserializer<Cacheable> BLOCK_DESERIALIZER = new CacheableDeserializer<Cacheable>() {
        ...
        @Override
        public HFileBlock deserialize(ByteBuff buf, boolean reuse, MemoryType memType) throws IOException {
    ...
    private HFileBlock(HFileBlock that, boolean bufCopy) {
        init(that.blockType, that.onDiskSizeWithoutHeader, that.uncompressedSizeWithoutHeader, that.prevBlockOffset, that.offset, that.onDiskDataSizeWithHeader, that.nextBlockOnDiskSize, that.fileContext);
        if (bufCopy) {
     ...
     */
    @VisibleForTesting
    public HFileBlock(BlockType blockType, int onDiskSizeWithoutHeader, int uncompressedSizeWithoutHeader, long prevBlockOffset, ByteBuffer b, boolean fillHeader, long offset, final int nextBlockOnDiskSize, int onDiskDataSizeWithHeader, HFileContext fileContext) {
     ...
     */
    HFileBlock(ByteBuff buf, boolean usesHBaseChecksum, MemoryType memType, final long offset, final int nextBlockOnDiskSize, HFileContext fileContext) throws IOException {
        buf.rewind();
     ...
     */
    private void init(BlockType blockType, int onDiskSizeWithoutHeader, int uncompressedSizeWithoutHeader, long prevBlockOffset, long offset, int onDiskDataSizeWithHeader, final int nextBlockOnDiskSize, HFileContext fileContext) {
    ...
    public long heapSize() {
        long size = ClassSize.align(ClassSize.OBJECT + // Block type, multi byte buffer, MemoryType and meta references
        4 * ClassSize.REFERENCE + // On-disk size, uncompressed size, and next block's on-disk size
        // bytePerChecksum and onDiskDataSize
        4 * Bytes.SIZEOF_INT + // This and previous block offset
        2 * Bytes.SIZEOF_LONG + // Heap size of the meta object. meta will be always not null.
    ...
    static class Writer implements ShipperListener {
        ...
        DataOutputStream startWriting(BlockType newBlockType) throws IOException {
            ...
            userDataStream = ((ByteBufferWriterDataOutputStream) org.zlab.ocov.tracker.Runtime.update(new ByteBufferWriterDataOutputStream(baosInMemory), 154, newBlockType));
        ...
        void write(Cell cell) throws IOException {
            ...
            this.unencodedDataSizeWritten += this.dataBlockEncoder.encode(cell, dataBlockEncodingCtx, this.userDataStream);
        ...
        private void finishBlock() throws IOException {
            ...
            if (compressAndEncryptDat == null) {
                compressAndEncryptDat = ((Bytes) org.zlab.ocov.tracker.Runtime.update(new Bytes(baosInMemory.getBuffer(), 0, baosInMemory.size()), 155));
            }
            if (onDiskBlockBytesWithHeader == null) {
                onDiskBlockBytesWithHeader = ((ByteArrayOutputStream) org.zlab.ocov.tracker.Runtime.update(new ByteArrayOutputStream(compressAndEncryptDat.getLength()), 156));
            }
        ...
        ByteBuffer cloneUncompressedBufferWithHeader() {
            ...
            int numBytes = (int) ChecksumUtil.numBytes(onDiskBlockBytesWithHeader.size(), fileContext.getBytesPerChecksum());
            putHeader(uncompressedBlockBytesWithHeader, 0, onDiskBlockBytesWithHeader.size() + numBytes, baosInMemory.size(), onDiskBlockBytesWithHeader.size());
    ...
    static class FSReaderImpl implements FSReader {
        ...
        public HFileBlock readBlockData(long offset, long onDiskSizeWithHeaderL, boolean pread, boolean updateMetrics) throws IOException {
            ...
            HFileBlock blk = readBlockDataInternal(is, offset, onDiskSizeWithHeaderL, pread, doVerificationThruHBaseChecksum, updateMetrics);
            if (blk == null) {
                ...
                blk = readBlockDataInternal(is, offset, onDiskSizeWithHeaderL, pread, doVerificationThruHBaseChecksum, updateMetrics);
                if (blk != null) {
                    HFile.LOG.warn("HDFS checksum verification succeeded for file " + pathName + " at offset " + offset + " filesize " + fileSize);
                }
            }
         ...
         * @throws IOException
         */
        private static int checkAndGetSizeAsInt(final long onDiskSizeWithHeaderL, final int hdrSize) throws IOException {
         ...
         * @throws IOException
         */
        private void verifyOnDiskSizeMatchesHeader(final int passedIn, final ByteBuffer headerBuf, final long offset, boolean verifyChecksum) throws IOException {
         ...
         * @see PrefetchedHeader
         */
        private void cacheNextBlockHeader(final long offset, final byte[] header, final int headerOffset, final int headerLength) {
        ...
        protected HFileBlock readBlockDataInternal(FSDataInputStream is, long offset, long onDiskSizeWithHeaderL, boolean pread, boolean verifyChecksum, boolean updateMetrics) throws IOException {
            ...
            if (onDiskSizeWithHeader <= 0) {
                ...
                if (headerBuf == null) {
                    ...
                    readAtOffset(is, headerBuf.array(), headerBuf.arrayOffset(), hdrSize, false, offset, pread);
                }
            ...
            HFileBlock hFileBlock = new HFileBlock(new SingleByteBuff(onDiskBlockByteBuffer), checksumSupport, MemoryType.EXCLUSIVE, offset, nextBlockOnDiskSize, fileContext);
