...
public final class BloomFilterUtil {
     ...
     */
    public static final byte[] bitvals = { (byte) 0x01, (byte) 0x02, (byte) 0x04, (byte) 0x08, (byte) 0x10, (byte) 0x20, (byte) 0x40, (byte) 0x80 };
    ...
    public static long computeMaxKeys(long bitSize, double errorRate, int hashCount) {
        return (long) (-bitSize * 1.0 / hashCount * Math.log(1 - Math.exp(Math.log(errorRate) / hashCount)));
    }
    ...
    private static <T> boolean contains(ByteBuff bloomBuf, int bloomOffset, int bloomSize, Hash hash, int hashCount, HashKey<T> hashKey) {
        ...
        for (int i = 0; i < hashCount; i++) {
            int hashLoc = (randomGeneratorForTest == null ? // Production mode
            Math.abs(compositeHash % bloomBitSize) : // Test mode with "fake look-ups" to estimate "ideal false positive rate"
    ...
    public static String toString(BloomFilterChunk bloomFilter) {
        return formatStats(bloomFilter) + STATS_RECORD_SEP + "Actual error rate: " + String.format("%.8f", bloomFilter.actualErrorRate());
    }
}
