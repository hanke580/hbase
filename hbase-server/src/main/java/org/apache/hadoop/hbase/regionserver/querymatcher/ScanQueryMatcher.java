...
public abstract class ScanQueryMatcher implements ShipperListener {
     ...
     */
    private static boolean isCellTTLExpired(final Cell cell, final long oldestTimestamp, final long now) {
    ...
    public void setToNewRow(Cell currentRow) {
        ...
        org.zlab.ocov.tracker.Runtime.update(this, 271, currentRow);
        columns.reset();
        reset();
    }
    ...
    public int compareKeyForNextRow(Cell nextIndexed, Cell currentCell) {
        return PrivateCellUtil.compareKeyBasedOnColHint(rowComparator, nextIndexed, currentCell, 0, 0, null, 0, 0, HConstants.OLDEST_TIMESTAMP, Type.Minimum.getCode());
    }
    ...
    public int compareKeyForNextColumn(Cell nextIndexed, Cell currentCell) {
        ...
        if (nextColumn == null) {
            ...
            return PrivateCellUtil.compareKeyBasedOnColHint(rowComparator, nextIndexed, currentCell, currentCell.getFamilyOffset(), currentCell.getFamilyLength(), nextColumn.getBuffer(), nextColumn.getOffset(), nextColumn.getLength(), HConstants.LATEST_TIMESTAMP, Type.Maximum.getCode());
        }
    }
    ...
    public void beforeShipped() throws IOException {
        if (this.currentRow != null) {
            ...
            org.zlab.ocov.tracker.Runtime.update(this, 762);
        }
        if (columns != null) {
    ...
    protected static Pair<DeleteTracker, ColumnTracker> getTrackers(RegionCoprocessorHost host, NavigableSet<byte[]> columns, ScanInfo scanInfo, long oldestUnexpiredTS, Scan userScan) throws IOException {
        ...
        if (scanInfo.isNewVersionBehavior() && (userScan == null || !userScan.isRaw())) {
            ...
            deleteTracker = ((ScanDeleteTracker) org.zlab.ocov.tracker.Runtime.update(new ScanDeleteTracker(scanInfo.getComparator()), 518, host, columns, scanInfo, oldestUnexpiredTS, userScan));
        }
        if (host != null) {
        ...
        if (deleteTracker instanceof NewVersionBehaviorTracker) {
            ...
            columnTracker = new ScanWildcardColumnTracker(scanInfo.getMinVersions(), maxVersionToCheck, oldestUnexpiredTS, scanInfo.getComparator());
        } else {
            columnTracker = ((ExplicitColumnTracker) org.zlab.ocov.tracker.Runtime.update(new ExplicitColumnTracker(columns, scanInfo.getMinVersions(), maxVersionToCheck, oldestUnexpiredTS), 517, host, columns, scanInfo, oldestUnexpiredTS, userScan));
        }
