...
public abstract class ScanQueryMatcher implements ShipperListener {
    ...
    public void setToNewRow(Cell currentRow) {
        ...
        org.zlab.ocov.tracker.Runtime.update(this, 84, currentRow);
        columns.reset();
        reset();
    }
    ...
    protected static Pair<DeleteTracker, ColumnTracker> getTrackers(RegionCoprocessorHost host, NavigableSet<byte[]> columns, ScanInfo scanInfo, long oldestUnexpiredTS, Scan userScan) throws IOException {
        ...
        if (scanInfo.isNewVersionBehavior() && (userScan == null || !userScan.isRaw())) {
            ...
            deleteTracker = ((ScanDeleteTracker) org.zlab.ocov.tracker.Runtime.update(new ScanDeleteTracker(scanInfo.getComparator()), 86, host, columns, scanInfo, oldestUnexpiredTS, userScan));
        }
        if (host != null) {
        ...
        if (deleteTracker instanceof NewVersionBehaviorTracker) {
            ...
            columnTracker = ((ExplicitColumnTracker) org.zlab.ocov.tracker.Runtime.update(new ExplicitColumnTracker(columns, scanInfo.getMinVersions(), maxVersionToCheck, oldestUnexpiredTS), 87, host, columns, scanInfo, oldestUnexpiredTS, userScan));
        }
