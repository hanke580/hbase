...
public final class PermissionStorage {
    ...
    public static void removeUserPermission(Configuration conf, UserPermission userPerm, Table t) throws IOException {
        if (null == userPerm.getPermission().getActions() || userPerm.getPermission().getActions().length == 0) {
            ...
            List<UserPermission> permsList = getUserPermissions(conf, userPermissionRowKey(userPerm.getPermission()), null, null, null, false);
    ...
    static private void removeTablePermissions(TableName tableName, byte[] column, Table table, boolean closeTable) throws IOException {
        Scan scan = new Scan();
    ...
    static Map<byte[], ListMultimap<String, UserPermission>> loadAll(Region aclRegion) throws IOException {
        ...
        Scan scan = ((Scan) org.zlab.ocov.tracker.Runtime.update(new Scan(), 692, aclRegion));
        scan.addFamily(ACL_LIST_FAMILY);
    ...
    public static ListMultimap<String, UserPermission> getTablePermissions(Configuration conf, TableName tableName) throws IOException {
        return getPermissions(conf, tableName != null ? tableName.getName() : null, null, null, null, null, false);
    }
    ...
    public static ListMultimap<String, UserPermission> getNamespacePermissions(Configuration conf, String namespace) throws IOException {
        return getPermissions(conf, Bytes.toBytes(toNamespaceEntry(namespace)), null, null, null, null, false);
    }
    ...
    public static List<UserPermission> getUserNamespacePermissions(Configuration conf, String namespace, String user, boolean hasFilterUser) throws IOException {
        return getUserPermissions(conf, Bytes.toBytes(toNamespaceEntry(namespace)), null, null, user, hasFilterUser);
    }
    ...
    private static Pair<String, Permission> parsePermissionRecord(byte[] entryName, Cell kv, byte[] cf, byte[] cq, boolean filterPerms, String filterUser) {
        ...
        if (filterPerms) {
            if (username.charAt(0) == '@' && !StringUtils.isEmpty(filterUser) && filterUser.charAt(0) != '@') {
        ...
        if (isNamespaceEntry(entryName)) {
            ...
            return new Pair<>(username, Permission.newBuilder(Bytes.toString(fromNamespaceEntry(entryName))).withActionCodes(value).build());
        }
        // Handle global entry
        ...
        return new Pair<>(username, Permission.newBuilder(TableName.valueOf(entryName)).withFamily(permFamily).withQualifier(permQualifier).withActionCodes(value).build());
    }
