...
public class CloseRegionHandler extends EventHandler {
     ...
     * @param destination
     */
    public CloseRegionHandler(final Server server, final RegionServerServices rsServices, final RegionInfo regionInfo, final boolean abort, ServerName destination) {
        this(server, rsServices, regionInfo, abort, EventType.M_RS_CLOSE_REGION, destination);
    }

    protected CloseRegionHandler(final Server server, final RegionServerServices rsServices, RegionInfo regionInfo, boolean abort, EventType eventType, ServerName destination) {
    ...
    public void process() {
        try {
            ...
            rsServices.reportRegionStateTransition(((RegionStateTransitionContext) org.zlab.ocov.tracker.Runtime.update(new RegionStateTransitionContext(TransitionCode.CLOSED, HConstants.NO_SEQNUM, -1, regionInfo), 48)));
