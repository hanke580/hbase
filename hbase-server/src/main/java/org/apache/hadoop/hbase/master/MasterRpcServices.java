...
public class MasterRpcServices extends RSRpcServices implements MasterService.BlockingInterface, RegionServerStatusService.BlockingInterface, LockService.BlockingInterface, HbckService.BlockingInterface {
    ...
    protected RpcServerInterface createRpcServer(Server server, Configuration conf, RpcSchedulerFactory rpcSchedulerFactory, InetSocketAddress bindAddress, String name) throws IOException {
        ...
        boolean reservoirEnabled = conf.getBoolean(RESERVOIR_ENABLED_KEY, LoadBalancer.isMasterCanHostUserRegions(conf));
        try {
            return RpcServerFactory.createRpcServer(server, name, getServices(), // use final bindAddress for this server.
            bindAddress, conf, rpcSchedulerFactory.create(conf, this, server), reservoirEnabled);
    ...
    public GetSchemaAlterStatusResponse getSchemaAlterStatus(RpcController controller, GetSchemaAlterStatusRequest req) throws ServiceException {
        ...
        try {
            ...
            org.zlab.ocov.tracker.Runtime.update(pair, 141, controller, req);
    ...
    public IsCatalogJanitorEnabledResponse isCatalogJanitorEnabled(RpcController c, IsCatalogJanitorEnabledRequest req) throws ServiceException {
        return IsCatalogJanitorEnabledResponse.newBuilder().setValue(master.isCatalogJanitorEnabled()).build();
    }
    ...
    public IsCleanerChoreEnabledResponse isCleanerChoreEnabled(RpcController c, IsCleanerChoreEnabledRequest req) throws ServiceException {
        return IsCleanerChoreEnabledResponse.newBuilder().setValue(master.isCleanerChoreEnabled()).build();
    }
    ...
    public ModifyTableResponse modifyTable(RpcController controller, ModifyTableRequest req) throws ServiceException {
        try {
            long procId = master.modifyTable(ProtobufUtil.toTableName(req.getTableName()), ProtobufUtil.toTableDescriptor(req.getTableSchema()), req.getNonceGroup(), req.getNonce());
    ...
    public RestoreSnapshotResponse restoreSnapshot(RpcController controller, RestoreSnapshotRequest request) throws ServiceException {
        try {
            long procId = master.restoreSnapshot(request.getSnapshot(), request.getNonceGroup(), request.getNonce(), request.getRestoreACL());
    ...
    public UnassignRegionResponse unassignRegion(RpcController controller, UnassignRegionRequest req) throws ServiceException {
        try {
            ...
            Pair<RegionInfo, ServerName> pair = MetaTableAccessor.getRegion(master.getConnection(), regionName);
            if (Bytes.equals(RegionInfoBuilder.FIRST_META_REGIONINFO.getRegionName(), regionName)) {
                pair = new Pair<>(RegionInfoBuilder.FIRST_META_REGIONINFO, master.getMetaTableLocator().getMetaRegionLocation(master.getZooKeeper()));
            }
            if (pair == null) {
     ...
     */
    private long submitProcedure(HBaseProtos.RegionSpecifier rs, boolean override, BiFunction<RegionInfo, Boolean, Procedure> f) throws UnknownRegionException {
    ...
    public MasterProtos.AssignsResponse assigns(RpcController controller, MasterProtos.AssignsRequest request) throws ServiceException {
        ...
        try {
            ...
            for (HBaseProtos.RegionSpecifier rs : request.getRegionList()) {
                long pid = submitProcedure(rs, override, (r, b) -> this.master.getAssignmentManager().createAssignProcedure(r, b));
    ...
    public MasterProtos.UnassignsResponse unassigns(RpcController controller, MasterProtos.UnassignsRequest request) throws ServiceException {
        ...
        try {
            ...
            for (HBaseProtos.RegionSpecifier rs : request.getRegionList()) {
                long pid = submitProcedure(rs, override, (r, b) -> this.master.getAssignmentManager().createUnassignProcedure(r, b));
                responseBuilder.addPid(pid);
            }
            return responseBuilder.build();
        } catch (IOException ioe) {
    ...
    public MasterProtos.ScheduleServerCrashProcedureResponse scheduleServerCrashProcedure(RpcController controller, MasterProtos.ScheduleServerCrashProcedureRequest request) throws ServiceException {
        ...
        try {
            for (HBaseProtos.ServerName serverName : serverNames) {
                ...
                LOG.info("{} schedule ServerCrashProcedure for {}", master.getClientIdAuditPrefix(), server);
                if (shouldSubmitSCP(server)) {
                    master.getServerManager().moveFromOnlineToDeadServers(server);
                    ProcedureExecutor<MasterProcedureEnv> procExec = this.master.getMasterProcedureExecutor();
                    pids.add(procExec.submitProcedure(new ServerCrashProcedure(procExec.getEnvironment(), server, true, containMetaWals(server))));
                } else {
                    pids.add(-1L);
                }
            }
