...
public class TableStateManager {
    ...
    private void updateMetaState(TableName tableName, TableState.State newState) throws IOException {
        if (tableName.equals(TableName.META_TABLE_NAME)) {
            if (TableState.State.DISABLING.equals(newState) || TableState.State.DISABLED.equals(newState)) {
    ...
    private TableState readMetaState(TableName tableName) throws IOException {
        ...
        if (state != null) {
            return ((TableState) org.zlab.ocov.tracker.Runtime.update(new TableState(tableName, state), 186, tableName));
        }
        TableState tableState = MetaTableAccessor.getTableState(master.getConnection(), tableName);
        org.zlab.ocov.tracker.Runtime.update(tableState, 742, tableName);
        if (tableState != null) {
        ...
        try {
            for (Map.Entry<TableName, TableState.State> entry : ZKDataMigrator.queryForTableStates(this.master.getZooKeeper()).entrySet()) {
                ...
                if (ts == null) {
                    ...
                    if (zkstate.equals(TableState.State.ENABLED) || zkstate.equals(TableState.State.DISABLED)) {
                        ...
                        LOG.warn("Table={} has no state and zookeeper state is in-between={} (neither " + "ENABLED or DISABLED); NOT MIGRATING table state", entry.getKey(), zkstate);
                    }
                }
