...
public class HRegionFileSystem {
   ...
   */
    ...
    public static HRegionFileSystem createRegionOnFileSystem(final Configuration conf, final FileSystem fs, final Path tableDir, final RegionInfo regionInfo) throws IOException {
        HRegionFileSystem regionFs = ((HRegionFileSystem) org.zlab.ocov.tracker.Runtime.update(new HRegionFileSystem(conf, fs, tableDir, regionInfo), 490, conf, fs, tableDir, regionInfo));
    ...
    public static HRegionFileSystem openRegionFromFileSystem(final Configuration conf, final FileSystem fs, final Path tableDir, final RegionInfo regionInfo, boolean readOnly) throws IOException {
        HRegionFileSystem regionFs = ((HRegionFileSystem) org.zlab.ocov.tracker.Runtime.update(new HRegionFileSystem(conf, fs, tableDir, regionInfo), 218, conf, fs, tableDir, regionInfo, readOnly));
        Path regionDir = regionFs.getRegionDir();
        if (!fs.exists(regionDir)) {
