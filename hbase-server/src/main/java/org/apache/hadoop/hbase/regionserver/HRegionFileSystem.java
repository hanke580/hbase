...
public class HRegionFileSystem {
     ...
     */
    HRegionFileSystem(final Configuration conf, final FileSystem fs, final Path tableDir, final RegionInfo regionInfo) {
        this.fs = fs;
        this.conf = conf;
        ...
        this.baseSleepBeforeRetries = conf.getInt("hdfs.client.sleep.before.retries", DEFAULT_BASE_SLEEP_BEFORE_RETRIES);
    }
     ...
     */
    StoreFileInfo getStoreFileInfo(final String familyName, final String fileName) throws IOException {
        Path familyDir = getStoreDir(familyName);
        return ServerRegionReplicaUtil.getStoreFileInfo(conf, fs, regionInfo, regionInfoForFs, familyName, new Path(familyDir, fileName));
    }
    ...
    public void deleteFamily(final String familyName) throws IOException {
        ...
        if (fs.exists(familyDir) && !deleteDir(familyDir))
            throw new IOException("Could not delete family " + familyName + " from FileSystem for region " + regionInfoForFs.getRegionNameAsString() + "(" + regionInfoForFs.getEncodedName() + ")");
    }
     ...
     */
    private Path preCommitStoreFile(final String familyName, final Path buildPath, final long seqNum, final boolean generateNewName) throws IOException {
        Path storeDir = getStoreDir(familyName);
        if (!fs.exists(storeDir) && !createDir(storeDir))
            throw new IOException("Failed creating " + storeDir);
   ...
   */
    ...
    public Path splitStoreFile(RegionInfo hri, String familyName, HStoreFile f, byte[] splitRow, boolean top, RegionSplitPolicy splitPolicy) throws IOException {
        ...
        if (createLinkFile) {
            ...
            try {
                ...
                Path path = new Path(splitDir, HFileLink.createHFileLinkName(linkedTable, linkedRegion, hfileName));
    ...
    static boolean mkdirs(FileSystem fs, Configuration conf, Path dir) throws IOException {
        if (FSUtils.isDistributedFileSystem(fs) || !conf.getBoolean(HConstants.ENABLE_DATA_FILE_UMASK, false)) {
     ...
     */
    public void commitMergedRegion(List<Path> allMergedFiles, MasterProcedureEnv env) throws IOException {
     ...
     */
    private static void writeRegionInfoFileContent(final Configuration conf, final FileSystem fs, final Path regionInfoFile, final byte[] content) throws IOException {
    ...
    void checkRegionInfoOnFilesystem() throws IOException {
        ...
        try {
            ...
            LOG.warn(getRegionDir() + " doesn't exist for region: " + regionInfoForFs.getEncodedName() + " on table " + regionInfo.getTable());
        }
        try {
    ...
    public static HRegionFileSystem createRegionOnFileSystem(final Configuration conf, final FileSystem fs, final Path tableDir, final RegionInfo regionInfo) throws IOException {
        HRegionFileSystem regionFs = ((HRegionFileSystem) org.zlab.ocov.tracker.Runtime.update(new HRegionFileSystem(conf, fs, tableDir, regionInfo), 125, conf, fs, tableDir, regionInfo));
     ...
     * @throws IOException if the region creation fails due to a FileSystem exception.
     */
    public static HRegionFileSystem openRegionFromFileSystem(final Configuration conf, final FileSystem fs, final Path tableDir, final RegionInfo regionInfo, boolean readOnly) throws IOException {
        HRegionFileSystem regionFs = ((HRegionFileSystem) org.zlab.ocov.tracker.Runtime.update(new HRegionFileSystem(conf, fs, tableDir, regionInfo), 121, conf, fs, tableDir, regionInfo, readOnly));
        Path regionDir = regionFs.getRegionDir();
        if (!fs.exists(regionDir)) {
