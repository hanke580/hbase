...
public class KeyValueHeap extends NonReversedNonLazyKeyValueScanner implements KeyValueScanner, InternalScanner {
    ...
    public boolean next(List<Cell> result, ScannerContext scannerContext) throws IOException {
        ...
        org.zlab.ocov.tracker.Runtime.update(result, 49, result, scannerContext);
    ...
    public boolean seek(Cell seekKey) throws IOException {
        return // This is not a lazy seek
        generalizedSeek(// This is not a lazy seek
        false, seekKey, // forward (false: this is not a reseek)
    ...
    public boolean reseek(Cell seekKey) throws IOException {
        return // This is not a lazy seek
        generalizedSeek(// This is not a lazy seek
        false, seekKey, // forward (true because this is reseek)
        true, // Not using Bloom filters
        false);
    }
    ...
    private boolean generalizedSeek(boolean isLazy, Cell seekKey, boolean forward, boolean useBloom) throws IOException {
        ...
        try {
            while (scanner != null) {
                ...
                if (!seekResult) {
                    this.scannersForDelayedClose.add(scanner);
                    org.zlab.ocov.tracker.Runtime.update(this, 50, isLazy, seekKey, forward, useBloom);
                } else {
                    heap.add(scanner);
                }
                scanner = heap.poll();
