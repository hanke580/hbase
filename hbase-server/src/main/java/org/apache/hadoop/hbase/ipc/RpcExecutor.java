...
public abstract class RpcExecutor {
    ...
    public RpcExecutor(final String name, final int handlerCount, final int maxQueueLength, final PriorityFunction priority, final Configuration conf, final Abortable abortable) {
        this(name, handlerCount, conf.get(CALL_QUEUE_TYPE_CONF_KEY, CALL_QUEUE_TYPE_CONF_DEFAULT), maxQueueLength, priority, conf, abortable);
    }

    public RpcExecutor(final String name, final int handlerCount, final String callQueueType, final int maxQueueLength, final PriorityFunction priority, final Configuration conf, final Abortable abortable) {
        ...
        if (isDeadlineQueueType(callQueueType)) {
            ...
            queueInitArgs = new Object[] { maxQueueLength, codelTargetDelay, codelInterval, codelLifoThreshold, numGeneralCallsDropped, numLifoModeSwitches };
        ...
        LOG.info("Instantiated {} with queueClass={}; " + "numCallQueues={}, maxQueueLength={}, handlerCount={}", this.name, this.queueClass, this.numCallQueues, maxQueueLength, this.handlerCount);
    }
     ...
     */
    protected void startHandlers(final String nameSuffix, final int numHandlers, final List<BlockingQueue<CallRunner>> callQueues, final int qindex, final int qsize, final int port, final AtomicInteger activeHandlerCount) {
        ...
        for (int i = 0; i < numHandlers; i++) {
            ...
            Handler handler = getHandler(name, handlerFailureThreshhold, callQueues.get(index), activeHandlerCount);
            handler.start();
    ...
    protected class Handler extends Thread {
        ...
        Handler(final String name, final double handlerFailureThreshhold, final BlockingQueue<CallRunner> q, final AtomicInteger activeHandlerCount) {
            super(name);
            setDaemon(true);
            this.q = q;
        ...
        private void run(CallRunner cr) {
            ...
            cr.setStatus(status);
            org.zlab.ocov.tracker.Runtime.update(cr, 117, cr);
            try {
