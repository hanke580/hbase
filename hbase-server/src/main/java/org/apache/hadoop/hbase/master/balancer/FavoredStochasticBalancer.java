...
public class FavoredStochasticBalancer extends StochasticLoadBalancer implements FavoredNodesPromoter {
   ...
   */
    ...
    public Map<ServerName, List<RegionInfo>> roundRobinAssignment(List<RegionInfo> regions, List<ServerName> servers) throws HBaseIOException {
        ...
        try {
            FavoredNodeAssignmentHelper helper = new FavoredNodeAssignmentHelper(servers, fnm.getRackManager());
            helper.initialize();
   ...
   */
    class FavoredNodeLoadPicker extends CandidateGenerator {
        ...
        private int pickLeastLoadedFNServer(final BalancerClusterState cluster, List<ServerName> favoredNodes, int currentServerIndex) {
            List<Integer> fnIndex = new ArrayList<>();
            for (ServerName sn : favoredNodes) {
                if (cluster.serversToIndex.containsKey(sn.getAddress())) {
                    fnIndex.add(cluster.serversToIndex.get(sn.getAddress()));
            ...
            for (Entry<ServerName, List<RegionInfo>> entry : loadOfOneTable.entrySet()) {
                ...
                for (RegionInfo hri : entry.getValue()) {
                    ...
                    if (FavoredNodesPlan.getFavoredServerPosition(favoredNodes, current) != null || !FavoredNodesManager.isFavoredNodeApplicable(hri)) {
                        ...
                        regionPlans.add(rp);
                        org.zlab.ocov.tracker.Runtime.update(regionPlans, 198, tableName, loadOfOneTable);
                        misplacedRegions++;
                    }
                }
            }
