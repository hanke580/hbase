...
public abstract class RegionRemoteProcedureBase extends Procedure<MasterProcedureEnv> implements TableProcedureInterface, RemoteProcedure<MasterProcedureEnv, ServerName> {
    ...
    private RegionRemoteProcedureBaseState state = RegionRemoteProcedureBaseState.REGION_REMOTE_PROCEDURE_DISPATCH;
    ...
    public void remoteCallFailed(MasterProcedureEnv env, ServerName remote, IOException exception) {
        ...
        try {
            if (!env.getMasterServices().getServerManager().isServerOnline(remote)) {
                ...
                LOG.debug("{} for region {}, targetServer {} is dead, SCP will interrupt us, give up", this, regionNode, remote);
                return;
            }
            if (state != RegionRemoteProcedureBaseState.REGION_REMOTE_PROCEDURE_DISPATCH) {
                ...
                // procedure...
                LOG.warn("{} for region {}, targetServer={} has already been woken up, ignore", this, regionNode, remote);
                return;
            }
            LOG.warn("The remote operation {} for region {} to server {} failed", this, regionNode, remote, exception);
    ...
    protected abstract void checkTransition(RegionStateNode regionNode, TransitionCode transitionCode, long seqId) throws UnexpectedStateException;

    // change the in memory state of the regionNode, but do not update meta.
    protected abstract void updateTransitionWithoutPersistingToMeta(MasterProcedureEnv env, RegionStateNode regionNode, TransitionCode transitionCode, long seqId) throws IOException;
    ...
    private TransitRegionStateProcedure getParent(MasterProcedureEnv env) {
        return (TransitRegionStateProcedure) env.getMasterServices().getMasterProcedureExecutor().getProcedure(getParentProcId());
    }
    ...
    protected void serializeStateData(ProcedureStateSerializer serializer) throws IOException {
        RegionRemoteProcedureBaseStateData.Builder builder = RegionRemoteProcedureBaseStateData.newBuilder().setRegion(ProtobufUtil.toRegionInfo(region)).setTargetServer(ProtobufUtil.toServerName(targetServer)).setState(state);
        if (transitionCode != null) {
            builder.setTransitionCode(transitionCode);
            builder.setSeqId(seqId);
        }
        serializer.serialize(((org.apache.hadoop.hbase.shaded.protobuf.generated.MasterProcedureProtos.RegionRemoteProcedureBaseStateData) org.zlab.ocov.tracker.Runtime.update(builder.build(), 157, serializer)));
    }

    @Override
    protected void deserializeStateData(ProcedureStateSerializer serializer) throws IOException {
        RegionRemoteProcedureBaseStateData data = serializer.deserialize(RegionRemoteProcedureBaseStateData.class);
