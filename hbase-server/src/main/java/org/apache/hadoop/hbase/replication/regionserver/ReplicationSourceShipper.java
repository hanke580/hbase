...
public class ReplicationSourceShipper extends Thread {
    ...
    public enum WorkerState {
        ...
        RUNNING,
        STOPPED,
        // The worker is done processing a recovered queue
        FINISHED
    }
    ...
    public ReplicationSourceShipper(Configuration conf, String walGroupId, PriorityBlockingQueue<Path> queue, ReplicationSourceInterface source) {
        ...
        this.getEntriesTimeout = // 20 seconds
        ...
        while (isActive()) {
            ...
            try {
                ...
                if (entryBatch == null) {
                    ...
                    source.getSourceMetrics().setAgeOfLastShippedOp(EnvironmentEdgeManager.currentTime(), walGroupId);
                    continue;
                }
        ...
        while (isActive()) {
            try {
                ...
                for (Entry entry : entries) {
                    ...
                    source.getSourceMetrics().setAgeOfLastShippedOpByTable(entry.getKey().getWriteTime(), tableName.getNameAsString());
                }
                ...
                if (LOG.isTraceEnabled()) {
                    LOG.trace("Replicated {} entries or {} operations in {} ms", entries.size(), entryBatch.getNbOperations(), (endTimeNs - startTimeNs) / 1000000);
                }
                break;
            } catch (Exception ex) {
    ...
    private boolean updateLogPosition(WALEntryBatch batch) {
        ...
        if (batch.isEndOfFile() || !batch.getLastWalPath().equals(currentPath) || batch.getLastWalPosition() != currentPosition) {
            source.getSourceManager().logPositionAndCleanOldLogs(source.getQueueId(), source.isRecovered(), batch);
            updated = true;
        }
    ...
    public void startup(UncaughtExceptionHandler handler) {
        ...
        Threads.setDaemonThreadRunning(this, name + ".replicationSource.shipper" + walGroupId + "," + source.getQueueId(), handler);
    }
