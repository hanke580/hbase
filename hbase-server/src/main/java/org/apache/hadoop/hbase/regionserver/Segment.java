...
public abstract class Segment implements MemStoreSizing {
    ...
    public final static long FIXED_OVERHEAD = ClassSize.align(ClassSize.OBJECT + // cellSet, comparator, memStoreLAB, memStoreSizing,
    5 * ClassSize.REFERENCE + // and timeRangeTracker
    Bytes.// minSequenceId
    SIZEOF_LONG + // tagsPresent
    Bytes.SIZEOF_BOOLEAN);

    public final static long DEEP_OVERHEAD = FIXED_OVERHEAD + ClassSize.ATOMIC_REFERENCE + ClassSize.CELL_SET + 2 * ClassSize.ATOMIC_LONG;
    ...
    public boolean shouldSeek(TimeRange tr, long oldestUnexpiredTS) {
        return !isEmpty() && (tr.isAllTime() || timeRangeTracker.includesTimeRange(tr)) && timeRangeTracker.getMax() >= oldestUnexpiredTS;
    }
    ...
    protected void updateMetaInfo(Cell cellToAdd, boolean succ, boolean mslabUsed, MemStoreSizing memstoreSizing) {
        ...
        if (memstoreSizing != null) {
            ...
            org.zlab.ocov.tracker.Runtime.update(memstoreSizing, 99, cellToAdd, succ, mslabUsed, memstoreSizing);
        }
