...
public class AssignmentManager {
    ...
    AssignmentManager(MasterServices master, MasterRegion masterRegion, RegionStateStore stateStore) {
        ...
        this.shouldAssignRegionsWithFavoredNodes = FavoredStochasticBalancer.class.isAssignableFrom(conf.getClass(HConstants.HBASE_MASTER_LOADBALANCER_CLASS, Object.class));
        this.assignDispatchWaitMillis = conf.getInt(ASSIGN_DISPATCH_WAIT_MSEC_CONF_KEY, DEFAULT_ASSIGN_DISPATCH_WAIT_MSEC);
        this.assignDispatchWaitQueueMaxSize = conf.getInt(ASSIGN_DISPATCH_WAITQ_MAX_CONF_KEY, DEFAULT_ASSIGN_DISPATCH_WAITQ_MAX);
        this.assignMaxAttempts = Math.max(1, conf.getInt(ASSIGN_MAX_ATTEMPTS, DEFAULT_ASSIGN_MAX_ATTEMPTS));
        this.assignRetryImmediatelyMaxAttempts = conf.getInt(ASSIGN_RETRY_IMMEDIATELY_MAX_ATTEMPTS, DEFAULT_ASSIGN_RETRY_IMMEDIATELY_MAX_ATTEMPTS);
        int ritChoreInterval = conf.getInt(RIT_CHORE_INTERVAL_MSEC_CONF_KEY, DEFAULT_RIT_CHORE_INTERVAL_MSEC);
        this.ritChore = new RegionInTransitionChore(ritChoreInterval);
        int deadRegionChoreInterval = conf.getInt(DEAD_REGION_METRIC_CHORE_INTERVAL_MSEC_CONF_KEY, DEFAULT_DEAD_REGION_METRIC_CHORE_INTERVAL_MSEC);
        ...
        minVersionToMoveSysTables = conf.get(MIN_VERSION_MOVE_SYS_TABLES_CONFIG, DEFAULT_MIN_VERSION_MOVE_SYS_TABLES_CONFIG);
    }
    ...
    public void start() throws IOException, KeeperException {
        ...
        // region node.
        try (ResultScanner scanner = masterRegion.getScanner(((Scan) org.zlab.ocov.tracker.Runtime.update(new Scan(), 701)).addFamily(HConstants.CATALOG_FAMILY))) {
            for (; ; ) {
                Result result = scanner.next();
                if (result == null) {
                    break;
                }
                RegionStateStore.visitMetaEntry((r, regionInfo, state, regionLocation, lastHost, openSeqNum) -> {
    ...
    private RegionInfo getRegionInfo(RegionStateNode rsn) {
        if (rsn.isSplit() && !rsn.getRegionInfo().isSplit()) {
            ...
            return RegionInfoBuilder.newBuilder(rsn.getRegionInfo()).setSplit(true).setOffline(true).build();
        } else {
    ...
    public List<RegionInfo> getTableRegions(TableName tableName, boolean excludeOfflinedSplitParents) {
        return getRegionStateNodes(tableName, excludeOfflinedSplitParents).map(this::getRegionInfo).collect(Collectors.toList());
    }
    ...
    public List<Pair<RegionInfo, ServerName>> getTableRegionsAndLocations(TableName tableName, boolean excludeOfflinedSplitParents) {
        return getRegionStateNodes(tableName, excludeOfflinedSplitParents).map(rsn -> Pair.newPair(getRegionInfo(rsn), rsn.getRegionLocation())).collect(Collectors.toList());
    }
    ...
    private TransitRegionStateProcedure createAssignProcedure(RegionInfo regionInfo, ServerName sn, boolean override) throws IOException {
        ...
        try {
            ...
            return regionNode.setProcedure(TransitRegionStateProcedure.assign(getProcedureEnvironment(), regionInfo, sn));
        } finally {
            regionNode.unlock();
        }
    }
    ...
    private TransitRegionStateProcedure createAssignProcedure(RegionStateNode regionNode, ServerName targetServer) {
        regionNode.lock();
        try {
            return regionNode.setProcedure(TransitRegionStateProcedure.assign(getProcedureEnvironment(), regionNode.getRegionInfo(), targetServer));
        } finally {
            regionNode.unlock();
        }
    }
    ...
    public Future<byte[]> assignAsync(RegionInfo regionInfo, ServerName sn) throws IOException {
        return ProcedureSyncWait.submitProcedure(master.getMasterProcedureExecutor(), createAssignProcedure(regionInfo, sn, false));
    }
    ...
    public Future<byte[]> moveAsync(RegionPlan regionPlan) throws HBaseIOException {
        TransitRegionStateProcedure proc = createMoveRegionProcedure(regionPlan.getRegionInfo(), regionPlan.getDestination());
    ...
    public Future<byte[]> balance(RegionPlan regionPlan) throws HBaseIOException {
        ...
        if (current == null || !current.equals(regionPlan.getSource())) {
            LOG.debug("Skip region plan {}, source server not match, current region location is {}", regionPlan, current == null ? "(null)" : current);
            return null;
        }
    ...
    public TransitRegionStateProcedure[] createRoundRobinAssignProcedures(List<RegionInfo> hris, List<ServerName> serversToExclude) {
        ...
        try {
            ...
            Map<ServerName, List<RegionInfo>> assignments = getBalancer().roundRobinAssignment(hris, this.master.getServerManager().createDestinationServersList(serversToExclude));
            // Return mid-method!
    ...
    public TransitRegionStateProcedure createOneAssignProcedure(RegionInfo ri, boolean override) {
        ...
        try {
            ...
            LOG.info("Failed {} assign, override={}" + (override ? "" : "; set override to by-pass state checks."), ri.getEncodedName(), override, ioe);
        }
        return trsp;
    }
    ...
    public TransitRegionStateProcedure createOneUnassignProcedure(RegionInfo ri, boolean override) {
        ...
        try {
            ...
            assert regionNode.getProcedure() == null;
            trsp = TransitRegionStateProcedure.unassign(getProcedureEnvironment(), regionNode.getRegionInfo());
            regionNode.setProcedure(trsp);
        } catch (IOException ioe) {
            // 'override' must be false here.
            LOG.info("Failed {} unassign, override=false; set override to by-pass state checks.", ri.getEncodedName(), ioe);
        } finally {
            regionNode.unlock();
        }
        return trsp;
    ...
    public TransitRegionStateProcedure[] createAssignProcedures(List<RegionInfo> hris) {
        return hris.stream().map(hri -> regionStates.getOrCreateRegionStateNode(hri)).map(regionNode -> createAssignProcedure(regionNode, null)).sorted(AssignmentManager::compare).toArray(TransitRegionStateProcedure[]::new);
    }
    ...
    private TransitRegionStateProcedure[] createAssignProcedures(Map<ServerName, List<RegionInfo>> assignments) {
        return assignments.entrySet().stream().flatMap(e -> e.getValue().stream().map(hri -> regionStates.getOrCreateRegionStateNode(hri)).map(regionNode -> createAssignProcedure(regionNode, e.getKey()))).sorted(AssignmentManager::compare).toArray(TransitRegionStateProcedure[]::new);
    }
    ...
    private TransitRegionStateProcedure forceCreateUnssignProcedure(RegionStateNode regionNode) {
        ...
        try {
            ...
            return regionNode.setProcedure(TransitRegionStateProcedure.unassign(getProcedureEnvironment(), regionNode.getRegionInfo()));
        } finally {
            regionNode.unlock();
        }
    }
    ...
    public TransitRegionStateProcedure[] createUnassignProceduresForDisabling(TableName tableName) {
        return regionStates.getTableRegionStateNodes(tableName).stream().map(this::forceCreateUnssignProcedure).filter(p -> p != null).toArray(TransitRegionStateProcedure[]::new);
    }
     ...
     */
    public int submitUnassignProcedureForClosingExcessRegionReplicas(TableName tableName, int newReplicaCount, Consumer<TransitRegionStateProcedure> submit) {
            ...
            try {
                if (regionNode.getRegionInfo().getReplicaId() >= newReplicaCount) {
                    if (regionNode.isInTransition()) {
                        LOG.debug("skip scheduling unassign procedure for {} when closing excess region " + "replicas since it is in transition", regionNode);
                        inTransitionCount++;
                        continue;
                    }
                    if (regionNode.isInState(State.OFFLINE, State.CLOSED, State.SPLIT)) {
                    ...
                    submit.accept(regionNode.setProcedure(TransitRegionStateProcedure.unassign(getProcedureEnvironment(), regionNode.getRegionInfo())));
                    org.zlab.ocov.tracker.Runtime.update(regionNode, 465, tableName, newReplicaCount, submit);
                }
            } finally {
                regionNode.unlock();
            }
    ...
    public SplitTableRegionProcedure createSplitProcedure(final RegionInfo regionToSplit, final byte[] splitKey) throws IOException {
    ...
    // ============================================================================================
    private void reportRegionStateTransition(ReportRegionStateTransitionResponse.Builder builder, ServerName serverName, List<RegionStateTransition> transitionList) throws IOException {
        for (RegionStateTransition transition : transitionList) {
            switch(transition.getTransitionCode()) {
                ...
                case CLOSED:
                    ...
                    long procId = transition.getProcIdCount() > 0 ? transition.getProcId(0) : Procedure.NO_PROC_ID;
                    updateRegionTransition(serverName, transition.getTransitionCode(), hri, transition.hasOpenSeqNum() ? transition.getOpenSeqNum() : HConstants.NO_SEQNUM, procId);
                    break;
                case READY_TO_SPLIT:
                case SPLIT:
                case SPLIT_REVERTED:
                ...
                case MERGE_REVERTED:
                    ...
                    updateRegionMergeTransition(serverName, transition.getTransitionCode(), merged, mergeA, mergeB);
                    break;
            }
        }
    }
    ...
    private void updateRegionTransition(ServerName serverName, TransitionCode state, RegionInfo regionInfo, long seqId, long procId) throws IOException {
        ...
        LOG.trace("Update region transition serverName={} region={} regionState={}", serverName, regionNode, state);
        ServerStateNode serverNode = regionStates.getOrCreateServer(serverName);
        regionNode.lock();
        try {
            if (!reportTransition(regionNode, serverNode, state, seqId, procId)) {
                ...
                if (this.master.getServerManager().isClusterShutdown() && state.equals(TransitionCode.CLOSED)) {
                    LOG.info("RegionServer {} {}", state, regionNode.getRegionInfo().getEncodedName());
                } else {
                    LOG.warn("No matching procedure found for {} transition on {} to {}", serverName, regionNode, state);
                }
            }
        } finally {
            regionNode.unlock();
    ...
    private boolean reportTransition(RegionStateNode regionNode, ServerStateNode serverNode, TransitionCode state, long seqId, long procId) throws IOException {
        ServerName serverName = serverNode.getServerName();
        TransitRegionStateProcedure proc = regionNode.getProcedure();
        if (proc == null) {
            return false;
        }
        proc.reportTransition(master.getMasterProcedureExecutor().getEnvironment(), regionNode, serverName, state, seqId, procId);
        return true;
    }
    ...
    private void updateRegionMergeTransition(final ServerName serverName, final TransitionCode state, final RegionInfo merged, final RegionInfo hriA, final RegionInfo hriB) throws IOException {
    ...
    public void reportOnlineRegions(ServerName serverName, Set<byte[]> regionNames) {
        ...
        if (LOG.isTraceEnabled()) {
            LOG.trace("ReportOnlineRegions {} regionCount={}, metaLoaded={} {}", serverName, regionNames.size(), isMetaLoaded(), regionNames.stream().map(Bytes::toStringBinary).collect(Collectors.toList()));
        }
        ServerStateNode serverNode = regionStates.getOrCreateServer(serverName);
    ...
    private void checkOnlineRegionsReport(ServerStateNode serverNode, Set<byte[]> regionNames) {
        ...
        for (byte[] regionName : regionNames) {
            ...
            if (regionNode == null) {
                ...
                LOG.warn("No RegionStateNode for {} but reported as up on {}; closing...", regionNameAsStr, serverName);
            ...
            if (regionNode.tryLock()) {
                try {
                    ...
                    if (regionNode.isInState(State.OPENING, State.OPEN)) {
                        ...
                        if (!regionNode.getRegionLocation().equals(serverName) && diff > lag) {
                            LOG.warn("Reporting {} server does not match {} (time since last " + "update={}ms); closing...", serverName, regionNode, diff);
                            closeRegionSilently(serverNode.getServerName(), regionName);
                        }
                        ...
                        if (diff > lag) {
                            LOG.warn("Reporting {} state does not match {} (time since last update={}ms)", serverName, regionNode, diff);
                        }
                    }
                } finally {
                    regionNode.unlock();
                ...
                LOG.warn("Unable to acquire lock for regionNode {}. It is likely that another thread is currently holding the lock. To avoid deadlock, skip execution for now.", regionNode);
            }
        }
    }
        ...
        protected void periodicExecute(final MasterProcedureEnv env) {
            ...
            for (RegionStateNode rsn : am.getRegionStates().getRegionStateNodes()) {
                if (rsn.getState() != State.OPEN) {
                    // Opportunistic check, should quickly skip RITs, offline tables, etc.
                    continue;
                }
                ...
                if (state != State.OPEN) {
                    // Mostly skipping RITs that are already being take care of.
                    continue;
                }
                if (sn == null) {
                    // Opened on null?
            ...
            if (deadRegions > 0 || unknownRegions > 0) {
                LOG.info("Found {} OPEN regions on dead servers and {} OPEN regions on unknown servers", deadRegions, unknownRegions);
            }
    ...
    public static class RegionInTransitionStat {
        ...
        public RegionInTransitionStat(final Configuration conf) {
            this.ritThreshold = conf.getInt(METRICS_RIT_STUCK_WARNING_THRESHOLD, DEFAULT_RIT_STUCK_WARNING_THRESHOLD);
        }
        ...
        protected void update(final AssignmentManager am) {
            ...
            if (LOG.isDebugEnabled() && ritsOverThreshold != null && !ritsOverThreshold.isEmpty()) {
                LOG.debug("RITs over threshold: {}", ritsOverThreshold.entrySet().stream().map(e -> e.getKey() + ":" + e.getValue().getState().name()).collect(Collectors.joining("\n")));
            }
        }
    ...
    public void joinCluster() throws IOException {
        ...
        while (master.getServerManager().countOfRegionServers() < 1) {
            LOG.info("Waiting for RegionServers to join; current count={}", master.getServerManager().countOfRegionServers());
            Threads.sleep(250);
        }
    ...
    public void processOfflineRegions() {
        TransitRegionStateProcedure[] procs = regionStates.getRegionStateNodes().stream().filter(rsn -> rsn.isInState(State.OFFLINE)).filter(rsn -> isTableEnabled(rsn.getRegionInfo().getTable())).map(rsn -> {
            rsn.lock();
            try {
                if (rsn.getProcedure() != null) {
                    return null;
                } else {
                    return rsn.setProcedure(TransitRegionStateProcedure.assign(getProcedureEnvironment(), rsn.getRegionInfo(), null));
                }
            } finally {
                rsn.unlock();
            }
   ...
   */
    private class RegionMetaLoadingVisitor implements RegionStateStore.RegionStateVisitor {
        ...
        @Override
        public void visitRegionState(Result result, final RegionInfo regionInfo, final State state, final ServerName regionLocation, final ServerName lastHost, final long openSeqNum) {
            if (state == null && regionLocation == null && lastHost == null && openSeqNum == SequenceId.NO_SEQUENCE_ID) {
            ...
            if (localState.matches(State.OPEN, State.OPENING, State.CLOSING, State.SPLITTING, State.MERGING)) {
     ...
     */
    public void populateRegionStatesFromMeta(@NonNull final RegionInfo regionInfo) throws IOException {
        final String regionEncodedName = RegionInfo.DEFAULT_REPLICA_ID == regionInfo.getReplicaId() ? regionInfo.getEncodedName() : RegionInfoBuilder.newBuilder(regionInfo).setReplicaId(RegionInfo.DEFAULT_REPLICA_ID).build().getEncodedName();
        ...
        try {
            ...
            if (!force && serverNode != null && !serverNode.isInState(ServerState.ONLINE)) {
                ...
                if (force) {
                    ...
                    pid = procExec.submitProcedure(((ServerCrashProcedure) org.zlab.ocov.tracker.Runtime.update(new ServerCrashProcedure(mpe, serverName, shouldSplitWal, carryingMeta), 891, serverName, shouldSplitWal, force)));
                }
    ...
    public Map<ServerName, List<RegionInfo>> getSnapShotOfAssignment(final Collection<RegionInfo> regions) {
    ...
    public RegionInfo getRegionInfo(final String encodedRegionName) {
        final RegionStateNode regionState = regionStates.getRegionStateNodeFromEncodedRegionName(encodedRegionName);
        return regionState != null ? regionState.getRegionInfo() : null;
    }
    ...
    // ============================================================================================
    private static final State[] STATES_EXPECTED_ON_OPEN = { // Normal case
    State.OPENING, // Retrying
    State.OPEN };

    private static final State[] STATES_EXPECTED_ON_CLOSING = { // Normal case
    State.OPEN, // Retrying
    State.CLOSING, // Offline the split parent
    State.SPLITTING, // Offline the merge parents
    State.MERGING };

    private static final State[] STATES_EXPECTED_ON_CLOSED = { // Normal case
    ...
    // ============================================================================================
    public void markRegionAsSplit(final RegionInfo parent, final ServerName serverName, final RegionInfo daughterA, final RegionInfo daughterB) throws IOException {
        ...
        if (shouldAssignFavoredNodes(parent)) {
            ...
            ((FavoredNodesPromoter) getBalancer()).generateFavoredNodesForDaughter(onlineServers, parent, daughterA, daughterB);
        }
    }
    ...
    public void markRegionAsMerged(final RegionInfo child, final ServerName serverName, RegionInfo[] mergeParents) throws IOException {
        ...
        if (shouldAssignFavoredNodes(child)) {
            ((FavoredNodesPromoter) getBalancer()).generateFavoredNodesForMergedRegion(child, mergeParents);
        }
    }
   ...
   */
    ...
    protected void queueAssign(final RegionStateNode regionNode) {
        ...
        try {
            ...
            if (regionNode.isSystemTable() || pendingAssignQueue.size() == 1 || pendingAssignQueue.size() >= assignDispatchWaitQueueMaxSize) {
    ...
    private void processAssignQueue() {
        ...
        if (!systemHRIs.isEmpty()) {
            ...
            processAssignmentPlans(regions, null, systemHRIs, serversForSysTables.isEmpty() && !containsBogusAssignments(regions, systemHRIs) ? servers : serversForSysTables);
        }
    ...
    private boolean containsBogusAssignments(Map<RegionInfo, RegionStateNode> regions, List<RegionInfo> hirs) {
        for (RegionInfo ri : hirs) {
            if (regions.get(ri).getRegionLocation() != null && regions.get(ri).getRegionLocation().equals(LoadBalancer.BOGUS_SERVER_NAME)) {
                return true;
            }
        }
        return false;
    ...
    private void processAssignmentPlans(final HashMap<RegionInfo, RegionStateNode> regions, final HashMap<RegionInfo, ServerName> retainMap, final List<RegionInfo> hris, final List<ServerName> servers) {
    ...
    private void addToPendingAssignment(final HashMap<RegionInfo, RegionStateNode> regions, final Collection<RegionInfo> pendingRegions) {
        assignQueueLock.lock();
        try {
