...
public class AuthenticationTokenSecretManager extends SecretManager<AuthenticationTokenIdentifier> {
   ...
   */
    ...
    public byte[] retrievePassword(AuthenticationTokenIdentifier identifier) throws InvalidToken {
        ...
        if (masterKey == null) {
            if (zkWatcher.getWatcher().isAborted()) {
                ...
                throw new InvalidToken("Token keys could not be sync from zookeeper" + " because of ZKWatcher abort");
            }
            synchronized (this) {
    ...
    synchronized void rollCurrentKey() {
        ...
        AuthenticationKey newKey = new AuthenticationKey(++idSeq, // don't allow to expire until it's replaced by a new key
        Long.MAX_VALUE, generateSecret());
    ...
    private class LeaderElector extends Thread implements Stoppable {
        ...
        public LeaderElector(ZKWatcher watcher, String serverName) {
            ...
            zkLeader = new ZKLeaderManager(watcher, ZNodePaths.joinZNode(zkWatcher.getRootKeyZNode(), "keymaster"), Bytes.toBytes(serverName), this);
        }
