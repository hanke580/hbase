...
public class WALFactory {
    ...
    enum Providers {
        ...
        defaultProvider(AsyncFSWALProvider.class), filesystem(FSHLogProvider.class), multiwal(RegionGroupingProvider.class), asyncfs(AsyncFSWALProvider.class);
    ...
    public Class<? extends WALProvider> getProviderClass(String key, String defaultValue) {
        try {
            ...
            // all versions.
            if (provider == getDefaultProvider() && provider.clazz == AsyncFSWALProvider.class && !AsyncFSWALProvider.load()) {
    ...
    public WAL getWAL(RegionInfo region) throws IOException {
        ...
        if (region != null && region.isMetaRegion() && region.getReplicaId() == RegionInfo.DEFAULT_REPLICA_ID) {
            return ((org.apache.hadoop.hbase.wal.WALProvider) org.zlab.ocov.tracker.Runtime.update(getMetaProvider(), 330, region)).getWAL(region);
        } else {
     ...
     */
    public Reader createReader(final FileSystem fs, final Path path, CancelableProgressable reporter) throws IOException {
        return createReader(fs, path, reporter, true);
    }

    public Reader createReader(final FileSystem fs, final Path path, CancelableProgressable reporter, boolean allowCustom) throws IOException {
     ...
     */
    public static Reader createReader(final FileSystem fs, final Path path, final Configuration configuration) throws IOException {
     ...
     * @return a WAL Reader, caller must close.
     */
    static Reader createReader(final FileSystem fs, final Path path, final Configuration configuration, final CancelableProgressable reporter) throws IOException {
     ...
     * @return a WAL Reader, caller must close.
     */
    public static Reader createReaderIgnoreCustomClass(final FileSystem fs, final Path path, final Configuration configuration) throws IOException {
     ...
     */
    public static Writer createWALWriter(final FileSystem fs, final Path path, final Configuration configuration) throws IOException {
