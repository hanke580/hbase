...
public class WALCoprocessorHost extends CoprocessorHost<WALCoprocessor, WALCoprocessorEnvironment> {
     ...
     */
    static class WALEnvironment extends BaseEnvironment<WALCoprocessor> implements WALCoprocessorEnvironment {
         ...
         * @param wal      WAL
         */
        private WALEnvironment(final WALCoprocessor impl, final int priority, final int seq, final Configuration conf, final WAL wal) {
    ...
    @Override
    public WALEnvironment createEnvironment(final WALCoprocessor instance, final int priority, final int seq, final Configuration conf) {
    ...
    public WALCoprocessor checkAndGetInstance(Class<?> implClass) throws IllegalAccessException, InstantiationException {
        if (WALCoprocessor.class.isAssignableFrom(implClass)) {
            ...
            LOG.error(implClass.getName() + " is not of type WALCoprocessor. Check the " + "configuration " + CoprocessorHost.WAL_COPROCESSOR_CONF_KEY);
            return null;
        }
    }

    private ObserverGetter<WALCoprocessor, WALObserver> walObserverGetter = WALCoprocessor::getWALObserver;
