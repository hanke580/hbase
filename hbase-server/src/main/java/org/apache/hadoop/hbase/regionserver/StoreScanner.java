...
public class StoreScanner extends NonReversedNonLazyKeyValueScanner implements KeyValueScanner, InternalScanner, ChangedReadersObserver {
    ...
    public StoreScanner(HStore store, ScanInfo scanInfo, Scan scan, NavigableSet<byte[]> columns, long readPt) throws IOException {
        ...
        try {
            ...
            scanners = selectScannersFrom(store, store.getScanners(cacheBlocks, scanUsePread, false, matcher, scan.getStartRow(), scan.includeStartRow(), scan.getStopRow(), scan.includeStopRow(), this.readPt));
    ...
    protected List<KeyValueScanner> selectScannersFrom(HStore store, List<? extends KeyValueScanner> allScanners) {
        ...
        for (KeyValueScanner kvs : allScanners) {
            ...
            if (kvs.shouldUseScanner(scan, store, expiredTimestampCutoff)) {
                ...
                kvs.close();
                org.zlab.ocov.tracker.Runtime.update(kvs, 90, store, allScanners);
            }
        }
        return scanners;
    }
            ...
            switch(qcode) {
                ...
                case INCLUDE_AND_SEEK_NEXT_COL:
                    ...
                    if (this.countPerRow > storeOffset) {
                        ...
                        scannerContext.incrementSizeProgress(cellSize, PrivateCellUtil.estimatedSizeOfCell(cell));
    ...
    protected final boolean reopenAfterFlush() throws IOException {
        ...
        org.zlab.ocov.tracker.Runtime.update(this, 91);
    ...
    void trySwitchToStreamRead() {
        ...
        this.heap = newHeap;
        org.zlab.ocov.tracker.Runtime.update(this, 92);
        resetQueryMatcher(lastTop);
    ...
    private void parallelSeek(final List<? extends KeyValueScanner> scanners, final Cell kv) throws IOException {
        ...
        for (KeyValueScanner scanner : scanners) {
            if (scanner instanceof StoreFileScanner) {
                ...
                scanner.seek(kv);
                org.zlab.ocov.tracker.Runtime.update(scanner, 93, scanners, kv);
                latch.countDown();
            }
        }
        try {
