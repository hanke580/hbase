...
public class StoreScanner extends NonReversedNonLazyKeyValueScanner implements KeyValueScanner, InternalScanner, ChangedReadersObserver {
    ...
    private StoreScanner(HStore store, Scan scan, ScanInfo scanInfo, int numColumns, long readPt, boolean cacheBlocks, ScanType scanType) {
        ...
        this.useRowColBloom = numColumns > 1 || (!get && numColumns == 1) && (store == null || store.getColumnFamilyDescriptor().getBloomFilterType() == BloomType.ROWCOL);
    ...
    public StoreScanner(HStore store, ScanInfo scanInfo, Scan scan, NavigableSet<byte[]> columns, long readPt) throws IOException {
        ...
        try {
            ...
            scanners = selectScannersFrom(store, store.getScanners(cacheBlocks, scanUsePread, false, matcher, scan.getStartRow(), scan.includeStartRow(), scan.getStopRow(), scan.includeStopRow(), this.readPt, isOnlyLatestVersionScan(scan)));
    ...
    public StoreScanner(ScanInfo scanInfo, ScanType scanType, List<? extends KeyValueScanner> scanners) throws IOException {
        ...
        assert scanType != ScanType.USER_SCAN;
        this.matcher = CompactionScanQueryMatcher.create(scanInfo, scanType, Long.MAX_VALUE, 0L, oldestUnexpiredTS, now, null, null, null);
    ...
    StoreScanner(Scan scan, ScanInfo scanInfo, NavigableSet<byte[]> columns, List<? extends KeyValueScanner> scanners, ScanType scanType) throws IOException {
        ...
        if (scanType == ScanType.USER_SCAN) {
            ...
            this.matcher = CompactionScanQueryMatcher.create(scanInfo, scanType, Long.MAX_VALUE, HConstants.OLDEST_TIMESTAMP, oldestUnexpiredTS, now, null, null, null);
        }
        seekAllScanner(scanInfo, scanners);
    }
    ...
    protected boolean trySkipToNextColumn(Cell cell) throws IOException {
        ...
        do {
            Cell nextIndexedKey = getNextIndexedKey();
            if (nextIndexedKey != null && nextIndexedKey != KeyValueScanner.NO_NEXT_INDEXED_KEY && (nextIndexedKey == previousIndexedKey || matcher.compareKeyForNextColumn(nextIndexedKey, cell) >= 0)) {
                this.heap.next();
                ++kvsScanned;
                previousIndexedKey = nextIndexedKey;
            } else {
    ...
    public void updateReaders(List<HStoreFile> sfs, List<KeyValueScanner> memStoreScanners) throws IOException {
        ...
        try {
            ...
            List<KeyValueScanner> scanners = store.getScanners(sfs, cacheBlocks, get, usePread, isCompaction, matcher, scan.getStartRow(), scan.getStopRow(), this.readPt, false, isOnlyLatestVersionScan(scan));
    ...
    protected final boolean reopenAfterFlush() throws IOException {
        ...
        org.zlab.ocov.tracker.Runtime.update(this, 85);
    ...
    void trySwitchToStreamRead() {
        if (readType != Scan.ReadType.DEFAULT || !scanUsePread || closing || heap.peek() == null || bytesRead < preadMaxBytes) {
            return;
        }
        LOG.debug("Switch to stream read (scanned={} bytes) of {}", bytesRead, this.store.getColumnFamilyName());
        scanUsePread = false;
        Cell lastTop = heap.peek();
        ...
        try {
            ...
            fileScanners = store.recreateScanners(scannersToClose, cacheBlocks, false, false, matcher, scan.getStartRow(), scan.includeStartRow(), scan.getStopRow(), scan.includeStopRow(), readPt, false);
    ...
    List<KeyValueScanner> getAllScannersForTesting() {
        ...
        for (KeyValueScanner scanner : heap.getHeap()) allScanners.add(scanner);
        return allScanners;
    }
