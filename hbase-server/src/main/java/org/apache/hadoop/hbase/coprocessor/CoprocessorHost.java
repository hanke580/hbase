...
public abstract class CoprocessorHost<C extends Coprocessor, E extends CoprocessorEnvironment<C>> {
    ...
    protected <O> boolean execOperation(final ObserverOperation<O> observerOperation) throws IOException {
        ...
        for (E env : envs) {
            ...
            org.zlab.ocov.tracker.Runtime.update(observerOperation, 169, observerOperation);
            Thread currentThread = Thread.currentThread();
    ...
    protected <O> boolean execShutdown(final ObserverOperation<O> observerOperation) throws IOException {
        ...
        for (E env : envs) {
            observerOperation.prepare(env);
            org.zlab.ocov.tracker.Runtime.update(observerOperation, 170, observerOperation);
            Thread currentThread = Thread.currentThread();
            ClassLoader cl = currentThread.getContextClassLoader();
            try {
                currentThread.setContextClassLoader(env.getClassLoader());
