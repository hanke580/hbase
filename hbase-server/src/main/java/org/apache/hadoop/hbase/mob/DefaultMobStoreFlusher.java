...
public class DefaultMobStoreFlusher extends DefaultStoreFlusher {
    ...
    public List<Path> flushSnapshot(MemStoreSnapshot snapshot, long cacheFlushId, MonitoredTask status, ThroughputController throughputController, FlushLifeCycleTracker tracker) throws IOException {
        ...
        try {
            ...
            synchronized (flushLock) {
                ...
                writer = store.createWriterInTmp(cellsCount, store.getColumnFamilyDescriptor().getCompressionType(), false, true, true, false);
                org.zlab.ocov.tracker.Runtime.update(writer, 6, snapshot, cacheFlushId, status, throughputController, tracker);
                IOException e = null;
                try {
    ...
    protected void performMobFlush(MemStoreSnapshot snapshot, long cacheFlushId, InternalScanner scanner, StoreFileWriter writer, MonitoredTask status, ThroughputController throughputController) throws IOException {
        ...
        mobFileWriter = mobStore.createWriterInTmp(new Date(time), snapshot.getCellsCount(), store.getColumnFamilyDescriptor().getCompressionType(), store.getRegionInfo().getStartKey(), false);
        org.zlab.ocov.tracker.Runtime.update(mobFileWriter, 7, snapshot, cacheFlushId, scanner, writer, status, throughputController);
