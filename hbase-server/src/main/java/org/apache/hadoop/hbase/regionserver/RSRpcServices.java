...
public class RSRpcServices implements HBaseRPCErrorHandler, AdminService.BlockingInterface, ClientService.BlockingInterface, PriorityFunction, ConfigurationObserver {
     ...
     */
    private boolean checkAndRowMutate(final HRegion region, final List<ClientProtos.Action> actions, final CellScanner cellScanner, byte[] row, byte[] family, byte[] qualifier, CompareOperator op, ByteArrayComparable comparator, TimeRange timeRange, RegionActionResult.Builder builder, ActivePolicyEnforcement spaceQuotaEnforcement) throws IOException {
        ...
        if (r == null) {
            ...
            try {
                ...
                if (canProceed) {
                    ...
                    org.zlab.ocov.tracker.Runtime.update(region, 100, region, quota, mutation, cellScanner, nonceGroup, spaceQuota);
                } else {
    ...
    private Result increment(final HRegion region, final OperationQuota quota, final MutationProto mutation, final CellScanner cells, long nonceGroup, ActivePolicyEnforcement spaceQuota) throws IOException {
        ...
        if (r == null) {
            ...
            try {
                ...
                if (canProceed) {
                    ...
                    org.zlab.ocov.tracker.Runtime.update(region, 101, region, quota, mutation, cells, nonceGroup, spaceQuota);
                } else {
        ...
        if (regionServer.metricsRegionServer != null) {
            regionServer.metricsRegionServer.updateIncrement(region.getTableDescriptor().getTableName(), EnvironmentEdgeManager.currentTime() - before);
        }
        return r == null ? Result.EMPTY_RESULT : r;
    }
            ...
            try {
                ...
                if (action.hasGet()) {
                    ...
                    try {
                        ...
                        if (regionServer.metricsRegionServer != null) {
                            regionServer.metricsRegionServer.updateGet(region.getTableDescriptor().getTableName(), EnvironmentEdgeManager.currentTime() - before);
                        }
                    }
    ...
    private void doAtomicBatchOp(final RegionActionResult.Builder builder, final HRegion region, final OperationQuota quota, final List<ClientProtos.Action> mutations, final CellScanner cells, ActivePolicyEnforcement spaceQuotaEnforcement) throws IOException {
     ...
     * @param mutations
     */
    private void doBatchOp(final RegionActionResult.Builder builder, final HRegion region, final OperationQuota quota, final List<ClientProtos.Action> mutations, final CellScanner cells, ActivePolicyEnforcement spaceQuotaEnforcement, boolean atomic) throws IOException {
     ...
     * @throws IOException
     */
    private OperationStatus[] doReplayBatchOp(final HRegion region, final List<WALSplitter.MutationReplay> mutations, long replaySeqId) throws IOException {
        long before = EnvironmentEdgeManager.currentTime();
        boolean batchContainsPuts = false, batchContainsDelete = false;
        try {
                ...
                if (metaCells != null && !metaCells.isEmpty()) {
                    for (Cell metaCell : metaCells) {
                        ...
                        if (regionEvent != null && !isDefaultReplica) {
                            ...
                            org.zlab.ocov.tracker.Runtime.update(region, 102, region, mutations, replaySeqId);
                            continue;
                        }
    ...
    private RegionScannerHolder addScanner(String scannerName, RegionScanner s, Shipper shipper, HRegion r, boolean needCursor) throws LeaseStillHeldException {
        Lease lease = regionServer.leases.createLease(scannerName, this.scannerLeaseTimeoutPeriod, new ScannerListener(scannerName));
        RpcCallback shippedCallback = ((RegionScannerShippedCallBack) org.zlab.ocov.tracker.Runtime.update(new RegionScannerShippedCallBack(scannerName, shipper, lease), 103, scannerName, s, shipper, r, needCursor));
        RpcCallback closeCallback;
        if (s instanceof RpcCallback) {
            closeCallback = (RpcCallback) s;
        } else {
            closeCallback = ((RegionScannerCloseCallBack) org.zlab.ocov.tracker.Runtime.update(new RegionScannerCloseCallBack(s), 104, scannerName, s, shipper, r, needCursor));
        }
    ...
    protected List<BlockingServiceAndInterface> getServices() {
        ...
        return new org.apache.hbase.thirdparty.com.google.common.collect.ImmutableList.Builder<BlockingServiceAndInterface>().addAll(bssi).build();
    }
   ...
   */
            ...
            if (regionAction.hasAtomic() && regionAction.getAtomic()) {
                ...
                try {
                    if (request.hasCondition()) {
                        ...
                        processed = checkAndRowMutate(region, regionAction.getActionList(), cellScanner, row, family, qualifier, op, comparator, timeRange, regionActionResultBuilder, spaceQuotaEnforcement);
                    } else {
                ...
                if (context != null && closeCallBack == null) {
                    ...
                    // Set this only once
                    closeCallBack = ((RegionScannersCloseCallBack) org.zlab.ocov.tracker.Runtime.update(new RegionScannersCloseCallBack(), 105, rpcc, request));
    ...
    private void scan(HBaseRpcController controller, ScanRequest request, RegionScannerHolder rsh, long maxQuotaResultSize, int maxResults, int limitOfRows, List<Result> results, ScanResponse.Builder builder, MutableObject lastBlock, RpcCallContext context) throws IOException {
            ...
            synchronized (scanner) {
                ...
                while (numOfResults < maxResults) {
                    ...
                    if (!values.isEmpty()) {
                        ...
                        results.add(r);
                        org.zlab.ocov.tracker.Runtime.update(results, 106, controller, request, rsh, maxQuotaResultSize, maxResults, limitOfRows, results, builder, lastBlock, context);
                        numOfResults++;
                        ...
                        if (r.mayHaveMoreCellsInRow()) {
                            ...
                            org.zlab.ocov.tracker.Runtime.update(results, 107, controller, request, rsh, maxQuotaResultSize, maxResults, limitOfRows, results, builder, lastBlock, context);
                        }
                    }
        ...
        try {
            ...
            if (request.getProcCount() > 0) {
                for (RemoteProcedureRequest req : request.getProcList()) {
                    ...
                    try {
                        callable = Class.forName(req.getProcClass()).asSubclass(RSProcedureCallable.class).getDeclaredConstructor().newInstance();
                    } catch (Exception e) {
