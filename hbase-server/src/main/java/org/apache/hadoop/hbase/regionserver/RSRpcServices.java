...
public class RSRpcServices implements HBaseRPCErrorHandler, AdminService.BlockingInterface, ClientService.BlockingInterface, ClientMetaService.BlockingInterface, PriorityFunction, ConfigurationObserver {
   ...
   */
    ...
    private Result append(final HRegion region, final OperationQuota quota, final MutationProto mutation, final CellScanner cellScanner, long nonceGroup, ActivePolicyEnforcement spaceQuota) throws IOException {
        ...
        if (regionServer.getMetrics() != null) {
            regionServer.getMetrics().updateAppend(region.getTableDescriptor().getTableName(), EnvironmentEdgeManager.currentTime() - before);
        }
    ...
    private RegionScannerHolder addScanner(String scannerName, RegionScanner s, Shipper shipper, HRegion r, boolean needCursor, boolean fullRegionScan) throws LeaseStillHeldException {
        ...
        RpcCallback closeCallback = s instanceof RpcCallback ? (RpcCallback) s : ((RegionScannerCloseCallBack) org.zlab.ocov.tracker.Runtime.update(new RegionScannerCloseCallBack(s), 401, scannerName, s, shipper, r, needCursor, fullRegionScan));
   ...
   */
    ...
    private Result get(Get get, HRegion region, RegionScannersCloseCallBack closeCallBack, RpcCallContext context) throws IOException {
        ...
        Scan scan = ((Scan) org.zlab.ocov.tracker.Runtime.update(new Scan(get), 682, get, region, closeCallBack, context));
    ...
    public MutateResponse mutate(final RpcController rpcc, final MutateRequest request) throws ServiceException {
        ...
        try {
            ...
            return ((org.apache.hadoop.hbase.shaded.protobuf.generated.ClientProtos.MutateResponse) org.zlab.ocov.tracker.Runtime.update(builder.build(), 534, rpcc, request));
        } catch (IOException ie) {
    ...
    private CheckAndMutateResult checkAndMutate(HRegion region, OperationQuota quota, MutationProto mutation, CellScanner cellScanner, Condition condition, long nonceGroup, ActivePolicyEnforcement spaceQuota) throws IOException {
        ...
        org.zlab.ocov.tracker.Runtime.update(checkAndMutate, 939, region, quota, mutation, cellScanner, condition, nonceGroup, spaceQuota);
        long nonce = mutation.hasNonce() ? mutation.getNonce() : HConstants.NO_NONCE;
        ...
        if (rsh == null) {
            ...
            if (lastCallSeq != null) {
                ...
                if (request.hasNextCallSeq() && request.getNextCallSeq() != lastCallSeq + 1) {
                    throw new OutOfOrderScannerNextException("Expected nextCallSeq for closed request: " + (lastCallSeq + 1) + " But the nextCallSeq got from client: " + request.getNextCallSeq() + "; request=" + TextFormat.shortDebugString(request));
                } else {
    ...
    private Pair<String, RegionScannerHolder> newRegionScanner(ScanRequest request, ScanResponse.Builder builder) throws IOException {
        ...
        org.zlab.ocov.tracker.Runtime.update(scan, 912, request, builder);
    ...
    private void scan(HBaseRpcController controller, ScanRequest request, RegionScannerHolder rsh, long maxQuotaResultSize, int maxResults, int limitOfRows, List<Result> results, ScanResponse.Builder builder, MutableObject<Object> lastBlock, RpcCall rpcCall) throws IOException {
    ...
    public ScanResponse scan(final RpcController controller, final ScanRequest request) throws ServiceException {
        ...
        try {
            ...
            if (rows > 0) {
                ...
                if (!done) {
                    scan((HBaseRpcController) controller, request, rsh, maxQuotaResultSize, rows, limitOfRows, results, builder, lastBlock, rpcCall);
                } else {
