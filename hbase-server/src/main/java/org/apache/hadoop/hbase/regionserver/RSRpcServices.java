...
public class RSRpcServices implements HBaseRPCErrorHandler, AdminService.BlockingInterface, ClientService.BlockingInterface, ClientMetaService.BlockingInterface, BootstrapNodeService.BlockingInterface, PriorityFunction, ConfigurationObserver {
   ...
   */
    ...
    public static final String REGIONSERVER_BOOTSTRAP_NODES_SERVICE_CONFIG = "hbase.regionserver.bootstrap.nodes.executorService";
    ...
    private void addResult(final MutateResponse.Builder builder, final Result result, final HBaseRpcController rpcc, boolean clientCellBlockSupported) {
        if (result == null)
            return;
    ...
    private void addResults(ScanResponse.Builder builder, List<Result> results, HBaseRpcController controller, boolean isDefaultRegion, boolean clientCellBlockSupported) {
     ...
     */
    private Result append(final HRegion region, final OperationQuota quota, final MutationProto mutation, final CellScanner cellScanner, long nonceGroup, ActivePolicyEnforcement spaceQuota, RpcCallContext context) throws IOException {
        ...
        if (regionServer.getMetrics() != null) {
            long blockBytesScanned = context != null ? context.getBlockBytesScanned() - blockBytesScannedBefore : 0;
            regionServer.getMetrics().updateAppend(region, EnvironmentEdgeManager.currentTime() - before, blockBytesScanned);
        }
     ...
     */
    private Result increment(final HRegion region, final OperationQuota quota, final MutationProto mutation, final CellScanner cells, long nonceGroup, ActivePolicyEnforcement spaceQuota, RpcCallContext context) throws IOException {
        long before = EnvironmentEdgeManager.currentTime();
        ...
        if (metricsRegionServer != null) {
            long blockBytesScanned = context != null ? context.getBlockBytesScanned() - blockBytesScannedBefore : 0;
            metricsRegionServer.updateIncrement(region, EnvironmentEdgeManager.currentTime() - before, blockBytesScanned);
        }
        return r == null ? Result.EMPTY_RESULT : r;
    }
    ...
    private List<CellScannable> doNonAtomicRegionMutation(final HRegion region, final OperationQuota quota, final RegionAction actions, final CellScanner cellScanner, final RegionActionResult.Builder builder, List<CellScannable> cellsToReturn, long nonceGroup, final RegionScannersCloseCallBack closeCallBack, RpcCallContext context, ActivePolicyEnforcement spaceQuotaEnforcement) {
        ...
        for (ClientProtos.Action action : actions.getActionList()) {
            ...
            try {
                Result r = null;
                long blockBytesScannedBefore = context != null ? context.getBlockBytesScanned() : 0;
                if (context != null && context.isRetryImmediatelySupported() && (context.getResponseCellSize() > maxQuotaResultSize || blockBytesScannedBefore + context.getResponseExceptionSize() > maxQuotaResultSize)) {
                    ...
                    if (sizeIOE == null) {
                        ...
                        sizeIOE = new MultiActionResultTooLarge("Max size exceeded" + " CellSize: " + context.getResponseCellSize() + " BlockSize: " + blockBytesScannedBefore);
                ...
                if (action.hasGet()) {
                    ...
                    resultOrExceptionBuilder.setServiceResult(serviceResultBuilder.setValue(serviceResultBuilder.getValueBuilder().setName(result.getClass().getName()).// TODO: Copy!!!
                    ...
                    switch(type) {
                        case APPEND:
                            r = append(region, quota, action.getMutation(), cellScanner, nonceGroup, spaceQuotaEnforcement, context);
                            break;
                        case INCREMENT:
                            r = increment(region, quota, action.getMutation(), cellScanner, nonceGroup, spaceQuotaEnforcement, context);
                            break;
                        case PUT:
                        case DELETE:
    ...
    private void checkCellSizeLimit(final HRegion r, final Mutation m) throws IOException {
        if ((org.zlab.ocov.tracker.Runtime.updateBranch(r.maxCellSize, 0, ">", 4))) {
    ...
    private void doAtomicBatchOp(final RegionActionResult.Builder builder, final HRegion region, final OperationQuota quota, final List<ClientProtos.Action> mutations, final CellScanner cells, long nonceGroup, ActivePolicyEnforcement spaceQuotaEnforcement) throws IOException {
    ...
    private void doNonAtomicBatchOp(final RegionActionResult.Builder builder, final HRegion region, final OperationQuota quota, final List<ClientProtos.Action> mutations, final CellScanner cells, ActivePolicyEnforcement spaceQuotaEnforcement) {
        try {
            doBatchOp(builder, region, quota, mutations, cells, HConstants.NO_NONCE, spaceQuotaEnforcement, false);
        } catch (IOException e) {
            ...
            for (i = 0; i < codes.length; i++) {
                ...
                switch(codes[i].getOperationStatusCode()) {
                    ...
                    default:
                        ...
                        break;
                    case SUCCESS:
                        ClientProtos.Result result = codes[i].getResult() == null ? ClientProtos.Result.getDefaultInstance() : ProtobufUtil.toResult(codes[i].getResult());
    ...
    public RSRpcServices(final HRegionServer rs) throws IOException {
        ...
        // w/o the domain.
        final String name = rs.getProcessName() + "/" + Address.fromParts(initialIsa.getHostName(), initialIsa.getPort()).toStringWithoutDomain();
        ...
        minimumScanTimeLimitDelta = conf.getLong(REGION_SERVER_RPC_MINIMUM_SCAN_TIME_LIMIT_DELTA, DEFAULT_REGION_SERVER_RPC_MINIMUM_SCAN_TIME_LIMIT_DELTA);
    ...
    protected RpcServerInterface createRpcServer(final Server server, final RpcSchedulerFactory rpcSchedulerFactory, final InetSocketAddress bindAddress, final String name) throws IOException {
        ...
        try {
            ...
            throw new IOException(be.getMessage() + ". To switch ports use the '" + HConstants.REGIONSERVER_PORT + "' configuration property.", be.getCause() != null ? be.getCause() : be);
        }
    }
    ...
    private RegionScannerHolder addScanner(String scannerName, RegionScanner s, Shipper shipper, HRegion r, boolean needCursor, boolean fullRegionScan) throws LeaseStillHeldException {
        Lease lease = regionServer.getLeaseManager().createLease(scannerName, this.scannerLeaseTimeoutPeriod, new ScannerListener(scannerName));
    ...
    private boolean isFullRegionScan(Scan scan, HRegion region) {
        ...
        if (Bytes.compareTo(scan.getStartRow(), region.getRegionInfo().getStartKey()) <= 0 && (Bytes.compareTo(scan.getStopRow(), region.getRegionInfo().getEndKey()) >= 0 && !Bytes.equals(region.getRegionInfo().getEndKey(), HConstants.EMPTY_END_ROW) || Bytes.equals(scan.getStopRow(), HConstants.EMPTY_END_ROW))) {
            return true;
        }
        return false;
    }
    ...
    protected List<BlockingServiceAndInterface> getServices() {
        ...
        boolean bootstrapNodes = getConfiguration().getBoolean(REGIONSERVER_BOOTSTRAP_NODES_SERVICE_CONFIG, true);
        ...
        if (bootstrapNodes) {
            bssi.add(new BlockingServiceAndInterface(BootstrapNodeService.newReflectiveBlockingService(this), BootstrapNodeService.BlockingInterface.class));
        }
   ...
   */
    ...
    public FlushRegionResponse flushRegion(final RpcController controller, final FlushRegionRequest request) throws ServiceException {
        try {
            ...
            if (shouldFlush) {
                ...
                if (compactionNeeded) {
                    regionServer.getCompactSplitThread().requestSystemCompaction(region, "Compaction through user triggered flush");
                }
    ...
    public OpenRegionResponse openRegion(final RpcController controller, final OpenRegionRequest request) throws ServiceException {
        ...
        try {
            ...
            int timeout = regionServer.getConfiguration().getInt(HConstants.HBASE_RPC_TIMEOUT_KEY, HConstants.DEFAULT_HBASE_RPC_TIMEOUT) >> 2;
            long endTime = EnvironmentEdgeManager.currentTime() + timeout;
            synchronized (regionServer.online) {
                try {
                    while (EnvironmentEdgeManager.currentTime() <= endTime && !regionServer.isStopped() && !regionServer.isOnline()) {
        ...
        for (RegionOpenInfo regionOpenInfo : request.getOpenInfoList()) {
            ...
            try {
                ...
                final Boolean previous = regionServer.getRegionsInTransitionInRS().putIfAbsent(encodedNameBytes, Boolean.TRUE);
                    ...
                    if (regionServer.executorService == null) {
                        ...
                        if (region.isMetaRegion()) {
                            ...
                            if (htd.getPriority() >= HConstants.ADMIN_QOS || region.getTable().isSystemTable()) {
                                regionServer.executorService.submit(new OpenPriorityRegionHandler(regionServer, regionServer, region, htd, masterSystemTime));
                            } else {
    ...
    public ReplicateWALEntryResponse replicateWALEntry(final RpcController controller, final ReplicateWALEntryRequest request) throws ServiceException {
        try {
            ...
            if (regionServer.getReplicationSinkService() != null) {
                ...
                regionServer.getReplicationSinkService().replicateLogEntries(entries, cellScanner, request.getReplicationClusterId(), request.getSourceBaseNamespaceDirPath(), request.getSourceHFileArchiveDirPath());
    ...
    private com.google.protobuf.Message execServiceOnRegion(HRegion region, final ClientProtos.CoprocessorServiceCall serviceCall) throws IOException {
    ...
    private void failRegionAction(MultiResponse.Builder responseBuilder, RegionActionResult.Builder regionActionResultBuilder, RegionAction regionAction, CellScanner cellScanner, Throwable error) {
    ...
    public MultiResponse multi(final RpcController rpcc, final MultiRequest request) throws ServiceException {
        ...
        if (request.hasCondition()) {
            ...
            try {
                ...
                quota = getRpcQuotaManager().checkBatchQuota(region, regionAction.getActionList(), regionAction.hasCondition());
            } catch (IOException e) {
                ...
                if (regionAction.hasCondition()) {
                    try {
                        ...
                        if (regionAction.getActionCount() == 1) {
                            CheckAndMutateResult result = checkAndMutate(region, quota, regionAction.getAction(0).getMutation(), cellScanner, regionAction.getCondition(), nonceGroup, spaceQuotaEnforcement, context);
    ...
    public MutateResponse mutate(final RpcController rpcc, final MutateRequest request) throws ServiceException {
        ...
        try {
            ...
            ActivePolicyEnforcement spaceQuotaEnforcement = getSpaceQuotaManager().getActiveEnforcements();
            if (request.hasCondition()) {
                CheckAndMutateResult result = checkAndMutate(region, quota, mutation, cellScanner, request.getCondition(), nonceGroup, spaceQuotaEnforcement, context);
                ...
                switch(type) {
                    case APPEND:
                        ...
                        r = append(region, quota, mutation, cellScanner, nonceGroup, spaceQuotaEnforcement, context);
                        break;
                    case INCREMENT:
                        // TODO: this doesn't actually check anything.
                        r = increment(region, quota, mutation, cellScanner, nonceGroup, spaceQuotaEnforcement, context);
                        break;
                    case PUT:
    ...
    private void put(HRegion region, OperationQuota quota, MutationProto mutation, CellScanner cellScanner, ActivePolicyEnforcement spaceQuota) throws IOException {
        long before = EnvironmentEdgeManager.currentTime();
        Put put = ProtobufUtil.toPut(mutation, cellScanner);
        checkCellSizeLimit(region, put);
    ...
    private void delete(HRegion region, OperationQuota quota, MutationProto mutation, CellScanner cellScanner, ActivePolicyEnforcement spaceQuota) throws IOException {
        long before = EnvironmentEdgeManager.currentTime();
    ...
    private CheckAndMutateResult checkAndMutate(HRegion region, OperationQuota quota, MutationProto mutation, CellScanner cellScanner, Condition condition, long nonceGroup, ActivePolicyEnforcement spaceQuota, RpcCallContext context) throws IOException {
        long before = EnvironmentEdgeManager.currentTime();
        long blockBytesScannedBefore = context != null ? context.getBlockBytesScanned() : 0;
     ...
     *         value.
     */
    private Pair<String, RegionScannerHolder> newRegionScanner(ScanRequest request, ScanResponse.Builder builder) throws IOException {
        HRegion region = getRegion(request.getRegion());
        ...
        boolean fullRegionScan = !region.getRegionInfo().getTable().isSystemTable() && isFullRegionScan(scan, region);
        return new Pair<String, RegionScannerHolder>(scannerName, addScanner(scannerName, scanner, shipper, region, scan.isNeedCursorResult(), fullRegionScan));
    }
        ...
        if (allowHeartbeatMessages) {
            ...
            if (scannerLeaseTimeoutPeriod > 0 || remainingTimeout > 0) {
                ...
                if (scannerLeaseTimeoutPeriod > 0 && remainingTimeout > 0) {
                    ...
                    timeLimitDelta = scannerLeaseTimeoutPeriod > 0 ? scannerLeaseTimeoutPeriod : remainingTimeout;
                }
    ...
    private void checkLimitOfRows(int numOfCompleteRows, int limitOfRows, boolean moreRows, ScannerContext scannerContext, ScanResponse.Builder builder) {
    ...
    private void scan(HBaseRpcController controller, ScanRequest request, RegionScannerHolder rsh, long maxQuotaResultSize, int maxResults, int limitOfRows, List<Result> results, ScanResponse.Builder builder, RpcCall rpcCall) throws IOException {
        ...
        try {
            ...
            synchronized (scanner) {
                ...
                while (numOfResults < maxResults) {
                    ...
                    numOfNextRawCalls++;
                    if (!values.isEmpty()) {
                        if ((org.zlab.ocov.tracker.Runtime.updateBranch(limitOfRows, 0, ">", 5))) {
                ...
                if (trackMetrics) {
                    ...
                    scannerContext.getMetrics().countOfBlockBytesScanned.set(scannerContext.getBlockSizeProgress());
                    if (rpcCall != null) {
    ...
    public ScanResponse scan(final RpcController controller, final ScanRequest request) throws ServiceException {
        ...
        OperationQuota quota;
        try {
            quota = getRpcQuotaManager().checkScanQuota(region, request, maxScannerResultSize, rsh.getMaxBlockBytesScanned(), rsh.getPrevBlockBytesScannedDifference());
        } catch (IOException e) {
            addScannerLeaseBack(lease);
            throw new ServiceException(e);
        }
        ...
        try {
            ...
            if (rows > 0) {
                ...
                if (!done) {
                    scan((HBaseRpcController) controller, request, rsh, maxQuotaResultSize, rows, limitOfRows, results, builder, rpcCall);
                } else {
            ...
            addResults(builder, results, (HBaseRpcController) controller, RegionReplicaUtil.isDefaultReplica(region.getRegionInfo()), isClientCellBlockSupport(rpcCall));
    ...
    @Override
    @QosPriority(priority = HConstants.ADMIN_QOS)
    public SlowLogResponses getSlowLogResponses(final RpcController controller, final SlowLogResponseRequest request) {
    ...
    @Override
    @QosPriority(priority = HConstants.ADMIN_QOS)
    public SlowLogResponses getLargeLogResponses(final RpcController controller, final SlowLogResponseRequest request) {
        final NamedQueueRecorder namedQueueRecorder = this.regionServer.getNamedQueueRecorder();
        final List<SlowLogPayload> slowLogPayloads = getSlowLogPayloads(request, namedQueueRecorder);
        SlowLogResponses slowLogResponses = SlowLogResponses.newBuilder().addAllSlowLogPayloads(slowLogPayloads).build();
        return slowLogResponses;
    ...
    @Override
    public GetCachedFilesListResponse getCachedFilesList(RpcController controller, GetCachedFilesListRequest request) throws ServiceException {
    ...
    @Override
    public GetClusterIdResponse getClusterId(RpcController controller, GetClusterIdRequest request) throws ServiceException {
        return GetClusterIdResponse.newBuilder().setClusterId(regionServer.getClusterId()).build();
    }

    @Override
    public GetActiveMasterResponse getActiveMaster(RpcController controller, GetActiveMasterRequest request) throws ServiceException {
        GetActiveMasterResponse.Builder builder = GetActiveMasterResponse.newBuilder();
        regionServer.getActiveMaster().ifPresent(name -> builder.setServerName(ProtobufUtil.toServerName(name)));
        return builder.build();
    }
    ...
    public GetMastersResponse getMasters(RpcController controller, GetMastersRequest request) throws ServiceException {
        ...
        regionServer.getActiveMaster().ifPresent(activeMaster -> builder.addMasterServers(GetMastersResponseEntry.newBuilder().setServerName(ProtobufUtil.toServerName(activeMaster)).setIsActive(true)));
        regionServer.getBackupMasters().forEach(backupMaster -> builder.addMasterServers(GetMastersResponseEntry.newBuilder().setServerName(ProtobufUtil.toServerName(backupMaster)).setIsActive(false)));
        return builder.build();
    }

    @Override
    public GetMetaRegionLocationsResponse getMetaRegionLocations(RpcController controller, GetMetaRegionLocationsRequest request) throws ServiceException {
        GetMetaRegionLocationsResponse.Builder builder = GetMetaRegionLocationsResponse.newBuilder();
        Optional<List<HRegionLocation>> metaLocations = regionServer.getMetaRegionLocationCache().getMetaRegionLocations();
        metaLocations.ifPresent(hRegionLocations -> hRegionLocations.forEach(location -> builder.addMetaLocations(ProtobufUtil.toRegionLocation(location))));
        return builder.build();
    }

    @Override
    public final GetBootstrapNodesResponse getBootstrapNodes(RpcController controller, GetBootstrapNodesRequest request) throws ServiceException {
        int maxNodeCount = regionServer.getConfiguration().getInt(CLIENT_BOOTSTRAP_NODE_LIMIT, DEFAULT_CLIENT_BOOTSTRAP_NODE_LIMIT);
    ...
    @Override
    public GetAllBootstrapNodesResponse getAllBootstrapNodes(RpcController controller, GetAllBootstrapNodesRequest request) throws ServiceException {
        GetAllBootstrapNodesResponse.Builder builder = GetAllBootstrapNodesResponse.newBuilder();
        regionServer.getBootstrapNodes().forEachRemaining(server -> builder.addNode(ProtobufUtil.toServerName(server)));
        return builder.build();
    }

    private void setReloadableGuardrails(Configuration conf) {
        rowSizeWarnThreshold = conf.getInt(HConstants.BATCH_ROWS_THRESHOLD_NAME, HConstants.BATCH_ROWS_THRESHOLD_DEFAULT);
        rejectRowsWithSizeOverThreshold = conf.getBoolean(REJECT_BATCH_ROWS_OVER_THRESHOLD, DEFAULT_REJECT_BATCH_ROWS_OVER_THRESHOLD);
        maxScannerResultSize = conf.getLong(HConstants.HBASE_SERVER_SCANNER_MAX_RESULT_SIZE_KEY, HConstants.DEFAULT_HBASE_SERVER_SCANNER_MAX_RESULT_SIZE);
    }
}
