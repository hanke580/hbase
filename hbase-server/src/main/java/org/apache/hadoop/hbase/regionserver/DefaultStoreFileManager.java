...
class DefaultStoreFileManager implements StoreFileManager {
    ...
    public DefaultStoreFileManager(CellComparator cellComparator, Comparator<HStoreFile> storeFileComparator, Configuration conf, CompactionConfiguration comConf) {
        ...
        blockingFileCount = conf.getInt(HStore.BLOCKING_STOREFILES_KEY, HStore.DEFAULT_BLOCKING_STOREFILE_COUNT);
        enableLiveFileTracking = shouldEnableHistoricalCompactionFiles(conf);
        storeFiles = new StoreFileList(ImmutableList.of(), enableLiveFileTracking ? ImmutableList.of() : null);
    }
    ...
    public void loadFiles(List<HStoreFile> storeFiles) throws IOException {
        this.storeFiles = ((StoreFileList) org.zlab.ocov.tracker.Runtime.update(new StoreFileList(ImmutableList.sortedCopyOf(storeFileComparator, storeFiles), enableLiveFileTracking ? ImmutableList.sortedCopyOf(storeFileComparator, getLiveFiles(storeFiles)) : null), 306, storeFiles));
    }
    ...
    public void insertNewFiles(Collection<HStoreFile> sfs) throws IOException {
        storeFiles = new StoreFileList(ImmutableList.sortedCopyOf(storeFileComparator, Iterables.concat(storeFiles.all, sfs)), enableLiveFileTracking ? ImmutableList.sortedCopyOf(storeFileComparator, Iterables.concat(storeFiles.live, getLiveFiles(sfs))) : null);
    }
    ...
    public void addCompactionResults(Collection<HStoreFile> newCompactedfiles, Collection<HStoreFile> results) throws IOException {
        ...
        if (enableLiveFileTracking) {
            liveStoreFiles = ImmutableList.sortedCopyOf(storeFileComparator, Iterables.concat(Iterables.filter(storeFiles.live, sf -> !newCompactedfiles.contains(sf)), getLiveFiles(results)));
        }
        storeFiles = new StoreFileList(ImmutableList.sortedCopyOf(storeFileComparator, Iterables.concat(Iterables.filter(storeFiles.all, sf -> !newCompactedfiles.contains(sf)), results)), liveStoreFiles);
        ...
        compactedfiles = ImmutableList.sortedCopyOf(storeFileComparator, Iterables.concat(compactedfiles, newCompactedfiles));
    }

    @Override
    public void removeCompactedFiles(Collection<HStoreFile> removedCompactedfiles) {
        compactedfiles = compactedfiles.stream().filter(sf -> !removedCompactedfiles.contains(sf)).sorted(storeFileComparator).collect(ImmutableList.toImmutableList());
    }
