...
public class HBaseInterClusterReplicationEndpoint extends HBaseReplicationEndpoint {
   ...
   */
    ...
    public void init(Context context) throws IOException {
        ...
        org.zlab.ocov.tracker.Runtime.update(this, 897, context);
    ...
    public boolean replicate(ReplicateContext replicateContext) {
        ...
        if (numSinks == 0) {
            if ((EnvironmentEdgeManager.currentTime() - lastSinkFetchTime) >= (maxRetriesMultiplier * 1000)) {
                LOG.warn("No replication sinks found, returning without replicating. " + "The source should retry with the same set of edits. Not logging this again for " + "the next {} seconds.", maxRetriesMultiplier);
    ...
    protected int replicateEntries(List<Entry> entries, int batchIndex, int timeout) throws IOException {
        ...
        try {
            ...
            try {
                ReplicationProtbufUtil.replicateWALEntry(rrs, entries.toArray(new Entry[entries.size()]), replicationClusterId, baseNamespaceDir, hfileArchiveDir, timeout);
                if (LOG.isTraceEnabled()) {
    ...
    private int serialReplicateRegionEntries(List<Entry> entries, int batchIndex, int timeout) throws IOException {
    ...
    protected Callable<Integer> createReplicator(List<Entry> entries, int batchIndex, int timeout) {
        return isSerial ? () -> serialReplicateRegionEntries(entries, batchIndex, timeout) : () -> replicateEntries(entries, batchIndex, timeout);
    }
