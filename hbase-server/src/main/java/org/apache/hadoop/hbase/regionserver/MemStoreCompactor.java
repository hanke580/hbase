...
public class MemStoreCompactor {
    ...
    public static final long DEEP_OVERHEAD = ClassSize.align(ClassSize.OBJECT + 4 * ClassSize.REFERENCE + // compactingMemStore, versionedList, isInterrupted, strategy (the reference)
    ...
    Bytes.// compactionKVMax
    SIZEOF_INT + // isInterrupted (the internals)
    ...
    public MemStoreCompactor(CompactingMemStore compactingMemStore, MemoryCompactionPolicy compactionPolicy) throws IllegalArgumentIOException {
        ...
        this.compactionKVMax = compactingMemStore.getConfiguration().getInt(HConstants.COMPACTION_KV_MAX, HConstants.COMPACTION_KV_MAX_DEFAULT);
        initiateCompactionStrategy(compactionPolicy, compactingMemStore.getConfiguration(), compactingMemStore.getFamilyName());
    }
    ...
    public boolean start() throws IOException {
        ...
        org.zlab.ocov.tracker.Runtime.update(this, 80);
    ...
    private void doCompaction() {
        ...
        if (isInterrupted.get()) {
            ...
            // the compaction also doesn't start when interrupted
            return;
        }
    ...
    private ImmutableSegment createSubstitution(MemStoreCompactionStrategy.Action action) throws IOException {
        ...
        switch(action) {
            ...
            case MERGE_COUNT_UNIQUE_KEYS:
                iterator = new MemStoreMergerSegmentsIterator(versionedList.getStoreSegments(), compactingMemStore.getComparator(), compactionKVMax);
                result = SegmentFactory.instance().createImmutableSegmentByMerge(compactingMemStore.getConfiguration(), compactingMemStore.getComparator(), iterator, versionedList.getNumOfCells(), versionedList.getStoreSegments(), compactingMemStore.getIndexType(), action);
                iterator.close();
                break;
            default:
                // sanity check
