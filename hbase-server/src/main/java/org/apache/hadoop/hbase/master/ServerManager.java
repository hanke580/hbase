...
public class ServerManager {
    ...
    public void regionServerReport(ServerName sn, ServerMetrics sl) throws YouAreDeadException {
        ...
        org.zlab.ocov.tracker.Runtime.update(sn, 139, sn, sl);
        if (null == this.onlineServers.replace(sn, sl)) {
            ...
            if (!checkAndRecordNewServer(sn, sl)) {
                ...
                // Not recorded, so no need to move on
                return;
            }
        }
     ...
     */
    void findDeadServersAndProcess(Set<ServerCrashProcedure> deadServersFromPE, Set<ServerName> liveServersFromWALDir) {
     ...
     */
    private void checkClockSkew(final ServerName serverName, final long serverCurrentTime) throws ClockOutOfSyncException {
    ...
    boolean expire(ServerName serverName) {
        return this.onlineServers.containsKey(serverName) || this.deadservers.isDeadServer(serverName) || this.master.getAssignmentManager().getRegionStates().getServerNode(serverName) != null || this.master.getMasterWalManager().isWALDirectoryNameWithWALs(serverName);
    }
    ...
    public synchronized boolean expireServer(final ServerName serverName) {
        ...
        if (!expire(serverName)) {
            LOG.info("Skipping expire; {} is not online, not in deadservers, not in fs -- presuming " + "long gone server instance!", serverName);
            return false;
        }
        if (this.deadservers.isDeadServer(serverName)) {
            LOG.warn("Expiration called on {} but crash processing in progress, serverStateNode={}", serverName, this.master.getAssignmentManager().getRegionStates().getServerNode(serverName));
            return false;
        }
   ...
   */
    ...
    private int getMinToStart() {
        ...
        if (LoadBalancer.isTablesOnMaster(master.getConfiguration()) && LoadBalancer.isSystemTablesOnlyOnMaster(master.getConfiguration())) {
    ...
    public void waitForRegionServers(MonitoredTask status) throws InterruptedException {
        ...
        while (!this.master.isStopped() && !isClusterShutdown() && count < maxToStart && ((lastCountChange + interval) > now || timeout > slept || count < minToStart)) {
            // Log some info at every interval time or if there is a change
            if (oldCount != count || lastLogTime + interval < now) {
                lastLogTime = now;
                String msg = "Waiting on regionserver count=" + count + "; waited=" + slept + "ms, expecting min=" + minToStart + " server(s), max=" + getStrForMax(maxToStart) + " server(s), " + "timeout=" + timeout + "ms, lastChange=" + (lastCountChange - now) + "ms";
                LOG.info(msg);
    ...
    public boolean isRegionInServerManagerStates(final RegionInfo hri) {
        ...
        return (storeFlushedSequenceIdsByRegion.containsKey(encodedName) || flushedSequenceIdByRegion.containsKey(encodedName));
    }
