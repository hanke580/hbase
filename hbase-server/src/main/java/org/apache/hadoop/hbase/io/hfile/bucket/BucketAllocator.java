...
public final class BucketAllocator {
    ...
    public final static class Bucket {
        ...
        public void addAllocation(long offset) throws BucketAllocatorException {
            ...
            if (offset < 0 || offset % itemAllocationSize != 0)
                throw new BucketAllocatorException("Attempt to add allocation for bad offset: " + offset + " base=" + baseOffset + ", bucket size=" + itemAllocationSize);
    ...
    final class BucketSizeInfo {
        ...
        public String toString() {
            return MoreObjects.toStringHelper(this.getClass()).add("sizeIndex", sizeIndex).add("bucketSize", bucketSizes[sizeIndex]).toString();
        }
    }
    ...
    private static final int[] DEFAULT_BUCKET_SIZES = { 4 * 1024 + 1024, 8 * 1024 + 1024, 16 * 1024 + 1024, 32 * 1024 + 1024, 40 * 1024 + 1024, 48 * 1024 + 1024, 56 * 1024 + 1024, 64 * 1024 + 1024, 96 * 1024 + 1024, 128 * 1024 + 1024, 192 * 1024 + 1024, 256 * 1024 + 1024, 384 * 1024 + 1024, 512 * 1024 + 1024 };
    ...
    public BucketSizeInfo roundUpToBucketSizeInfo(int blockSize) {
        for (int i = 0; i < bucketSizes.length; ++i) if (blockSize <= bucketSizes[i])
    ...
    BucketAllocator(long availableSpace, int[] bucketSizes) throws BucketAllocatorException {
        ...
        if (buckets.length < this.bucketSizes.length)
            throw new BucketAllocatorException("Bucket allocator size too small (" + buckets.length + "); must have room for at least " + this.bucketSizes.length + " buckets");
     ...
     */
    BucketAllocator(long availableSpace, int[] bucketSizes, Map<BlockCacheKey, BucketEntry> map, LongAdder realCacheSize) throws BucketAllocatorException {
    ...
    static class IndexStatistics {
        ...
        public IndexStatistics(long free, long used, long itemSize, int fullBuckets, int completelyFreeBuckets, long wastedBytes, long fragmentationBytes) {
            setTo(free, used, itemSize, fullBuckets, completelyFreeBuckets, wastedBytes, fragmentationBytes);
        }
    ...
    void logDebugStatistics() {
        ...
        LOG.debug("  Free bytes={}; used bytes={}; total bytes={}; wasted bytes={}; fragmentation bytes={}; " + "completelyFreeBuckets={}", total.freeBytes(), total.usedBytes(), total.totalBytes(), total.wastedBytes(), total.fragmentationBytes(), total.completelyFreeBuckets());
        for (IndexStatistics s : stats) {
            LOG.debug("  Object size {}; used={}; free={}; total={}; wasted bytes={}; fragmentation bytes={}, " + "full buckets={}", s.itemSize(), s.usedCount(), s.freeCount(), s.totalCount(), s.wastedBytes(), s.fragmentationBytes(), s.fullBuckets());
        }
    }
    ...
    IndexStatistics[] getIndexStatistics(IndexStatistics grandTotal) {
        ...
        grandTotal.setTo(totalfree, totalused, 1, fullBuckets, completelyFreeBuckets, totalWasted, totalFragmented);
        return stats;
    }

    IndexStatistics[] getIndexStatistics() {
        IndexStatistics[] stats = new IndexStatistics[bucketSizes.length];
        for (int i = 0; i < stats.length; ++i) stats[i] = bucketSizeInfos[i].statistics();
        return stats;
    }
