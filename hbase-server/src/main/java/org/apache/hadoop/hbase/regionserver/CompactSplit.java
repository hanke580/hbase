...
public class CompactSplit implements CompactionRequester, PropagatingConfigurationObserver {
   ...
   */
    ...
    private void requestCompactionInternal(HRegion region, HStore store, String why, int priority, boolean selectNow, CompactionLifeCycleTracker tracker, CompactionCompleteTracker completeTracker, User user) throws IOException {
        ...
        pool.execute(((CompactionRunner) org.zlab.ocov.tracker.Runtime.update(new CompactionRunner(store, region, compaction, tracker, completeTracker, pool, user), 108, region, store, why, priority, selectNow, tracker, completeTracker, user)));
