...
public class CompactSplit implements CompactionRequester, PropagatingConfigurationObserver {
    ...
    private void createSplitExcecutors() {
        ...
        this.splits = (ThreadPoolExecutor) Executors.newFixedThreadPool(splitThreads, new ThreadFactoryBuilder().setNameFormat(n + "-splits-%d").setDaemon(true).build());
    }
    ...
    public String toString() {
        return "compactionQueue=(longCompactions=" + longCompactions.getQueue().size() + ":shortCompactions=" + shortCompactions.getQueue().size() + ")" + ", splitQueue=" + splits.getQueue().size();
    }
   ...
   */
    ...
    public synchronized void requestCompaction(HRegion region, String why, int priority, CompactionLifeCycleTracker tracker, User user) throws IOException {
        requestCompactionInternal(region, why, priority, true, tracker, getCompleteTracker(tracker, () -> region.getTableDescriptor().getColumnFamilyCount()), user);
    }
    ...
    private void requestCompactionInternal(HRegion region, String why, int priority, boolean selectNow, CompactionLifeCycleTracker tracker, CompactionCompleteTracker completeTracker, User user) throws IOException {
    ...
    protected void requestCompactionInternal(HRegion region, HStore store, String why, int priority, boolean selectNow, CompactionLifeCycleTracker tracker, CompactionCompleteTracker completeTracker, User user) throws IOException {
        ...
        pool.execute(((CompactionRunner) org.zlab.ocov.tracker.Runtime.update(new CompactionRunner(store, region, compaction, tracker, completeTracker, pool, user), 473, region, store, why, priority, selectNow, tracker, completeTracker, user)));
        if (LOG.isDebugEnabled()) {
    ...
    private Optional<CompactionContext> selectCompaction(HRegion region, HStore store, int priority, CompactionLifeCycleTracker tracker, CompactionCompleteTracker completeTracker, User user) throws IOException {
    ...
    private final class CompactionRunner implements Runnable {
        ...
        public CompactionRunner(HStore store, HRegion region, CompactionContext compaction, CompactionLifeCycleTracker tracker, CompactionCompleteTracker completeTracker, ThreadPoolExecutor parent, User user) {
            this.store = store;
            this.region = region;
        ...
        private void doCompaction(User user) {
            ...
            try {
                ...
                if (completed) {
                    ...
                    if (region.getCompactPriority() < Store.PRIORITY_USER && store.getCompactPriority() <= 0) {
        ...
        public void run() {
            try {
                org.zlab.ocov.tracker.Runtime.update(this, 898);
                ...
                if (LOG.isDebugEnabled()) {
                    LOG.debug("Remove under compaction mark for store: {}", store.getHRegion().getRegionInfo().getEncodedName() + ":" + store.getColumnFamilyName());
                }
    ...
    private String getStoreNameForUnderCompaction(HStore store) {
        return String.format("%s:%s", store.getHRegion() != null ? store.getHRegion().getRegionInfo().getEncodedName() : "", store.getColumnFamilyName());
    }
}
