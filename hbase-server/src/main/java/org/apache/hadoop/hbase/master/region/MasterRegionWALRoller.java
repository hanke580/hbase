...
public final class MasterRegionWALRoller extends AbstractWALRoller<Abortable> {
    ...
    protected void afterWALArchive(Path oldPath, Path newPath) {
        ...
        // directory.
        try {
            MasterRegionUtils.moveFilesUnderDir(fs, walArchiveDir, globalWALArchiveDir, archivedWALSuffix);
        } catch (IOException e) {
            LOG.warn("Failed to move archived wals from {} to global dir {}", walArchiveDir, globalWALArchiveDir, e);
        }
    }
    ...
    static MasterRegionWALRoller create(String name, Configuration conf, Abortable abortable, FileSystem fs, Path walRootDir, Path globalWALRootDir, String archivedWALSuffix, long rollPeriodMs, long flushSize) {
        ...
        return ((MasterRegionWALRoller) org.zlab.ocov.tracker.Runtime.update(new MasterRegionWALRoller(name, conf, abortable, fs, walRootDir, globalWALRootDir, archivedWALSuffix), 643, name, conf, abortable, fs, walRootDir, globalWALRootDir, archivedWALSuffix, rollPeriodMs, flushSize));
    }
}
