...
public class ProtobufLogReader extends ReaderBase {
    ...
    // 1MB
    static final int DEFAULT_WAL_TRAILER_WARN_SIZE = 1024 * 1024;
    ...
    public long trailerSize() {
        if (trailerPresent) {
            ...
            final long calculatedSize = (long) PB_WAL_COMPLETE_MAGIC.length + Bytes.SIZEOF_INT + trailer.getSerializedSize();
            final long expectedSize = fileLength - walEditsStopOffset;
            if (expectedSize != calculatedSize) {
                LOG.warn("After parsing the trailer, we expect the total footer to be {} bytes, but we " + "calculate it as being {}", expectedSize, calculatedSize);
            }
            return expectedSize;
        } else {
            return -1L;
    ...
    enum WALHdrResult {
        ...
        // stream is at EOF when method starts
        EOF,
        SUCCESS,
    ...
    public void reset() throws IOException {
        ...
        // We need a new decoder (at least).
        initAfterCompression(clsName);
    }
