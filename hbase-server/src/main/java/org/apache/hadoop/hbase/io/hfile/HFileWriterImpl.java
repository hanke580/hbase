...
public class HFileWriterImpl implements HFile.Writer {
    ...
    protected void finishInit(final Configuration conf) {
        ...
        blockWriter = new HFileBlock.Writer(blockEncoder, hFileContext, cacheConf.getByteBuffAllocator());
    ...
    private static byte[] getMinimumMidpointArray(ByteBuffer left, int leftOffset, int leftLength, ByteBuffer right, int rightOffset, int rightLength) {
        ...
        int minLength = leftLength < rightLength ? leftLength : rightLength;
        int diffIdx = 0;
        while (diffIdx < minLength && ByteBufferUtils.toByte(left, leftOffset + diffIdx) == ByteBufferUtils.toByte(right, rightOffset + diffIdx)) {
            diffIdx++;
        }
        ...
        if (diffIdx >= minLength) {
            ...
            if ((0xff & diffByte) < 0xff && (diffByte + 1) < (ByteBufferUtils.toByte(right, rightOffset + diffIdx) & 0xff)) {
                minMidpoint = new byte[diffIdx + 1];
