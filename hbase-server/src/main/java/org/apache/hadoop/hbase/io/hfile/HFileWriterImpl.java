...
public class HFileWriterImpl implements HFile.Writer {
    ...
    public HFileWriterImpl(final Configuration conf, CacheConfig cacheConf, Path path, FSDataOutputStream outputStream, CellComparator comparator, HFileContext fileContext) {
    ...
    protected void finishInit(final Configuration conf) {
        ...
        blockWriter = ((HFileBlock.Writer) org.zlab.ocov.tracker.Runtime.update(new HFileBlock.Writer(blockEncoder, hFileContext), 142, conf));
        // Data block index writer
        boolean cacheIndexesOnWrite = cacheConf.shouldCacheIndexesOnWrite();
        dataBlockIndexWriter = ((HFileBlockIndex.BlockIndexWriter) org.zlab.ocov.tracker.Runtime.update(new HFileBlockIndex.BlockIndexWriter(blockWriter, cacheIndexesOnWrite ? cacheConf : null, cacheIndexesOnWrite ? name : null), 143, conf));
    ...
    private void writeInlineBlocks(boolean closing) throws IOException {
        for (InlineBlockWriter ibw : inlineBlockWriters) {
            while (ibw.shouldWriteBlock(closing)) {
                ...
                org.zlab.ocov.tracker.Runtime.update(ibw, 144, closing);
                blockWriter.writeHeaderAndData(outputStream);
    ...
    protected void finishClose(FixedFileTrailer trailer) throws IOException {
        ...
        org.zlab.ocov.tracker.Runtime.update(trailer, 147, trailer);
