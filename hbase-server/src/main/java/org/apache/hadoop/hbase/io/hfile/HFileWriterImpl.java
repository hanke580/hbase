...
public class HFileWriterImpl implements HFile.Writer {
    ...
    protected void finishInit(final Configuration conf) {
        ...
        blockWriter = new HFileBlock.Writer(conf, blockEncoder, hFileContext, cacheConf.getByteBuffAllocator());
        // Data block index writer
        boolean cacheIndexesOnWrite = cacheConf.shouldCacheIndexesOnWrite();
        dataBlockIndexWriter = ((HFileBlockIndex.BlockIndexWriter) org.zlab.ocov.tracker.Runtime.update(new HFileBlockIndex.BlockIndexWriter(blockWriter, cacheIndexesOnWrite ? cacheConf : null, cacheIndexesOnWrite ? name : null, indexBlockEncoder), 250, conf));
    ...
    protected void checkBlockBoundary() throws IOException {
        ...
        if (blockWriter.encodedBlockSizeWritten() >= encodedBlockSizeLimit || blockWriter.blockSizeWritten() >= hFileContext.getBlocksize()) {
            finishBlock();
    ...
    private void writeInlineBlocks(boolean closing) throws IOException {
        for (InlineBlockWriter ibw : inlineBlockWriters) {
            while (ibw.shouldWriteBlock(closing)) {
                ...
                org.zlab.ocov.tracker.Runtime.update(ibw, 244, closing);
                totalUncompressedBytes += blockWriter.getUncompressedSizeWithHeader();
                if (cacheThisBlock) {
                    doCacheOnWrite(offset);
                }
            }
            org.zlab.ocov.tracker.Runtime.update(ibw, 245, closing);
        }
    }
