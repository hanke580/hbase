...
public class RegionProcedureStore extends ProcedureStoreBase {
    ...
    private void checkUnsupportedProcedure(Map<Class<?>, List<Procedure<?>>> procsByType) throws HBaseIOException {
        ...
        for (Class<?> clazz : UNSUPPORTED_PROCEDURES) {
            ...
            if (procs != null) {
                LOG.error("Unsupported procedure type {} found, please rollback your master to the old" + " version to finish them, and then try to upgrade again." + " See https://hbase.apache.org/book.html#upgrade2.2 for more details." + " The full procedure list: {}", clazz, procs);
    ...
    public void load(ProcedureLoader loader) throws IOException {
        ...
        long maxProcId = 0;
        try (RegionScanner scanner = region.getRegionScanner(((Scan) org.zlab.ocov.tracker.Runtime.update(new Scan(), 691, loader)).addColumn(PROC_FAMILY, PROC_QUALIFIER))) {
    ...
    public void update(Procedure<?> proc) {
        runWithoutRpcCall(() -> {
            try {
                ProcedureProtos.Procedure proto = ProcedureUtil.convertToProtoProcedure(proc);
                region.update(r -> r.put(new Put(Bytes.toBytes(proc.getProcId())).addColumn(PROC_FAMILY, PROC_QUALIFIER, proto.toByteArray())));
            } catch (IOException e) {
    ...
    public void delete(long procId) {
        try {
            region.update(r -> r.put(new Put(Bytes.toBytes(procId)).addColumn(PROC_FAMILY, PROC_QUALIFIER, EMPTY_BYTE_ARRAY)));
        } catch (IOException e) {
    ...
    public void cleanup() {
        ...
        try (RegionScanner scanner = region.getRegionScanner(((Scan) org.zlab.ocov.tracker.Runtime.update(new Scan(), 880)).addColumn(PROC_FAMILY, PROC_QUALIFIER).setReversed(true))) {
            ...
            while (moreRows) {
                ...
                cells.clear();
                if (cell.getValueLength() == 0) {
                    region.update(r -> r.delete(new Delete(cell.getRowArray(), cell.getRowOffset(), cell.getRowLength()).addFamily(PROC_FAMILY)));
                }
            }
        } catch (IOException e) {
