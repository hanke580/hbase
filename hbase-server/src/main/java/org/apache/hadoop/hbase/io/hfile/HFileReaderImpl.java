...
public abstract class HFileReaderImpl implements HFile.Reader, Configurable {
    ...
    public HFileReaderImpl(ReaderContext context, HFileInfo fileInfo, CacheConfig cacheConf, Configuration conf) throws IOException {
        ...
        this.fsBlockReader = new HFileBlock.FSReaderImpl(context, hfileContext, cacheConf.getByteBuffAllocator(), conf);
    ...
    public String toString() {
        return "reader=" + path.toString() + (!isFileInfoLoaded() ? "" : ", compression=" + trailer.getCompressionCodec().getName() + ", cacheConf=" + cacheConf + ", firstKey=" + toStringFirstKey() + ", lastKey=" + toStringLastKey()) + ", avgKeyLen=" + fileInfo.getAvgKeyLen() + ", avgValueLen=" + fileInfo.getAvgValueLen() + ", entries=" + trailer.getEntryCount() + ", length=" + context.getFileSize();
    }
    ...
    public Optional<Cell> getFirstKey() {
        ...
        return dataBlockIndexReader.isEmpty() ? Optional.empty() : Optional.of(dataBlockIndexReader.getRootBlockKey(0));
    }
    ...
    protected static class HFileScannerImpl implements HFileScanner {
        ...
        public HFileScannerImpl(final HFile.Reader reader, final boolean cacheBlocks, final boolean pread, final boolean isCompaction) {
            this.reader = reader;
        ...
        private final void checkTagsLen() {
            if (checkLen(this.currTagsLen)) {
                throw new IllegalStateException("Invalid currTagsLen " + this.currTagsLen + ". Block offset: " + curBlock.getOffset() + ", block length: " + this.blockBuffer.limit() + ", position: " + this.blockBuffer.position() + " (without header)." + " path=" + reader.getPath());
            }
        }
        ...
        protected int blockSeek(Cell key, boolean seekBefore) {
            ...
            do {
                ...
                if (checkKeyLen(klen) || checkLen(vlen)) {
                    throw new IllegalStateException("Invalid klen " + klen + " or vlen " + vlen + ". Block offset: " + curBlock.getOffset() + ", block length: " + blockBuffer.limit() + ", position: " + blockBuffer.position() + " (without header)." + " path=" + reader.getPath());
                }
                ...
                int comp = PrivateCellUtil.compareKeyIgnoresMvcc(reader.getComparator(), key, bufBackedKeyOnlyKv);
                ...
                if (this.reader.getFileContext().isIncludesTags()) {
                    ...
                    if (checkLen(tlen)) {
                        throw new IllegalStateException("Invalid tlen " + tlen + ". Block offset: " + curBlock.getOffset() + ", block length: " + blockBuffer.limit() + ", position: " + blockBuffer.position() + " (without header)." + " path=" + reader.getPath());
                    }
                ...
                if (comp == 0) {
                    if (seekBefore) {
                        if (lastKeyValueSize < 0) {
                            throw new IllegalStateException("blockSeek with seekBefore " + "at the first key of the block: key=" + CellUtil.getCellKeyAsString(key) + ", blockOffset=" + curBlock.getOffset() + ", onDiskSize=" + curBlock.getOnDiskSizeWithHeader() + ", path=" + reader.getPath());
                        }
        ...
        public int reseekTo(Cell key) throws IOException {
            ...
            if (isSeeked()) {
                ...
                if (compared < 1) {
                    ...
                    if (this.nextIndexedKey != null && (this.nextIndexedKey == KeyValueScanner.NO_NEXT_INDEXED_KEY || PrivateCellUtil.compareKeyIgnoresMvcc(reader.getComparator(), key, nextIndexedKey) < 0)) {
        ...
        public int seekTo(Cell key, boolean rewind) throws IOException {
            ...
            BlockWithScanInfo blockWithScanInfo = indexReader.loadDataBlockWithScanInfo(key, curBlock, cacheBlocks, pread, isCompaction, getEffectiveDataBlockEncoding(), reader);
        ...
        public boolean seekBefore(Cell key) throws IOException {
            HFileBlock seekToBlock = reader.getDataBlockIndexReader().seekToDataBlock(key, curBlock, cacheBlocks, pread, isCompaction, reader.getEffectiveEncodingInCache(isCompaction), reader);
            if (seekToBlock == null) {
                return false;
            }
         ...
         */
        @edu.umd.cs.findbugs.annotations.SuppressWarnings(value = "NP_NULL_ON_SOME_PATH", justification = "Yeah, unnecessary null check; could do w/ clean up")
        ...
        public Cell getCell() {
            ...
            if (blockBuffer.hasArray()) {
                ...
                if ((org.zlab.ocov.tracker.Runtime.updateBranch(currTagsLen, 0, ">", 1))) {
                    ret = new SizeCachedKeyValue(blockBuffer.array(), blockBuffer.arrayOffset() + blockBuffer.position(), cellBufSize, seqId, currKeyLen, rowLen);
                } else {
                    ret = new SizeCachedNoTagsKeyValue(blockBuffer.array(), blockBuffer.arrayOffset() + blockBuffer.position(), cellBufSize, seqId, currKeyLen, rowLen);
                }
            } else {
                ByteBuffer buf = blockBuffer.asSubByteBuffer(cellBufSize);
                if (buf.isDirect()) {
                    ret = currTagsLen > 0 ? new SizeCachedByteBufferKeyValue(buf, buf.position(), cellBufSize, seqId, currKeyLen, rowLen) : new SizeCachedNoTagsByteBufferKeyValue(buf, buf.position(), cellBufSize, seqId, currKeyLen, rowLen);
                } else {
                    if ((org.zlab.ocov.tracker.Runtime.updateBranch(currTagsLen, 0, ">", 2))) {
                        ret = new SizeCachedKeyValue(buf.array(), buf.arrayOffset() + buf.position(), cellBufSize, seqId, currKeyLen, rowLen);
                    } else {
                        ret = new SizeCachedNoTagsKeyValue(buf.array(), buf.arrayOffset() + buf.position(), cellBufSize, seqId, currKeyLen, rowLen);
                    }
                }
            }
            return ret;
        ...
        private void positionThisBlockBuffer() {
            try {
                ...
                LOG.error("Current pos = " + blockBuffer.position() + "; currKeyLen = " + currKeyLen + "; currValLen = " + currValueLen + "; block limit = " + blockBuffer.limit() + "; currBlock currBlockOffset = " + this.curBlock.getOffset() + "; path=" + reader.getPath());
                throw e;
            }
        }
        ...
        protected void readAndUpdateNewBlock(long firstDataBlockOffset) throws IOException {
            ...
            if (newBlock.getOffset() < 0) {
                ...
                throw new IOException("Invalid offset=" + newBlock.getOffset() + ", path=" + reader.getPath());
            }
        ...
        protected void updateCurrentBlock(HFileBlock newBlock) throws IOException {
            try {
                if (newBlock.getBlockType() != BlockType.DATA) {
                    throw new IllegalStateException("ScannerV2 works only on data blocks, got " + newBlock.getBlockType() + "; " + "HFileName=" + reader.getPath() + ", " + "dataBlockEncoder=" + reader.getDataBlockEncoding() + ", " + "isCompaction=" + isCompaction);
                }
     ...
     */
    private HFileBlock getCachedBlock(BlockCacheKey cacheKey, boolean cacheBlock, boolean useLock, boolean updateCacheMetrics, BlockType expectedBlockType, DataBlockEncoding expectedDataBlockEncoding) throws IOException {
    ...
    public HFileBlock readBlock(long dataBlockOffset, long onDiskBlockSize, final boolean cacheBlock, boolean pread, final boolean isCompaction, boolean updateCacheMetrics, BlockType expectedBlockType, DataBlockEncoding expectedDataBlockEncoding) throws IOException {
        return readBlock(dataBlockOffset, onDiskBlockSize, cacheBlock, pread, isCompaction, updateCacheMetrics, expectedBlockType, expectedDataBlockEncoding, false);
    }

    @Override
    public HFileBlock readBlock(long dataBlockOffset, long onDiskBlockSize, final boolean cacheBlock, boolean pread, final boolean isCompaction, boolean updateCacheMetrics, BlockType expectedBlockType, DataBlockEncoding expectedDataBlockEncoding, boolean cacheOnly) throws IOException {
        if (dataBlockIndexReader == null) {
        ...
        try {
            while (true) {
                ...
                if (cacheConf.shouldReadBlockFromCache(expectedBlockType) && !cacheOnly) {
                    ...
                    HFileBlock cachedBlock = getCachedBlock(cacheKey, cacheBlock, useLock, updateCacheMetrics, expectedBlockType, expectedDataBlockEncoding);
                    if (cachedBlock != null) {
                ...
                HFileBlock hfileBlock = fsBlockReader.readBlockData(dataBlockOffset, onDiskBlockSize, pread, !isCompaction, shouldUseHeap(expectedBlockType, cacheable));
                try {
                ...
                cacheConf.getBlockCache().ifPresent(cache -> {
                    if (cacheable && cacheConf.shouldCacheBlockOnRead(category)) {
                        ...
                        cache.cacheBlock(cacheKey, cacheCompressed ? hfileBlock : unpacked, cacheConf.isInMemory(), cacheOnly);
                    }
                });
    ...
    private void validateBlockType(HFileBlock block, BlockType expectedBlockType) throws IOException {
        ...
        if (actualBlockType != expectedBlockType) {
            throw new IOException("Expected block type " + expectedBlockType + ", " + "but got " + actualBlockType + ": " + block + ", path=" + path);
        }
    }
    ...
    public Optional<Cell> getLastKey() {
        return dataBlockIndexReader.isEmpty() ? Optional.empty() : Optional.of(fileInfo.getLastKeyCell());
    }
        ...
        protected void updateCurrentBlock(HFileBlock newBlock) throws CorruptHFileException {
            try {
                ...
                if (!DataBlockEncoding.isCorrectEncoder(dataBlockEncoder, dataBlockEncoderId)) {
                    ...
                    throw new CorruptHFileException("Encoder " + encoderCls + " doesn't support data block encoding " + DataBlockEncoding.getNameFromId(dataBlockEncoderId) + ",path=" + reader.getPath());
                }
                updateCurrBlockRef(newBlock);
    ...
    private DataInput getBloomFilterMetadata(BlockType blockType) throws IOException {
        if (blockType != BlockType.GENERAL_BLOOM_META && blockType != BlockType.DELETE_FAMILY_BLOOM_META) {
            throw new RuntimeException("Block Type: " + blockType.toString() + " is not supported, path=" + path);
        }
    ...
    public boolean isFileInfoLoaded() {
        // We load file info in constructor in version 2.
        return true;
    }
     ...
     */
    @Override
    public HFileScanner getScanner(Configuration conf, boolean cacheBlocks, final boolean pread, final boolean isCompaction) {
    ...
    protected boolean cacheIfCompactionsOff() {
        return (!StoreFileInfo.isReference(name) && !HFileLink.isHFileLink(name)) || !conf.getBoolean(HBASE_REGION_SERVER_ENABLE_COMPACTION, true);
    }
}
