        ...
        try {
            ...
            for (WALEntry entry : entries) {
                ...
                for (int i = 0; i < count; i++) {
                    ...
                    org.zlab.ocov.tracker.Runtime.update(cells, 13, entries, cells, replicationClusterId, sourceBaseNamespaceDirPath, sourceHFileArchiveDirPath);
                ...
                for (Entry<List<String>, Map<String, List<Pair<byte[], List<String>>>>> entry : bulkLoadsPerClusters.entrySet()) {
                    ...
                    if (bulkLoadHFileMap != null && !bulkLoadHFileMap.isEmpty()) {
                        if (LOG.isDebugEnabled()) {
                            LOG.debug("Started replicating bulk loaded data from cluster ids: {}.", entry.getKey().toString());
                        }
                        HFileReplicator hFileReplicator = new HFileReplicator(this.provider.getConf(this.conf, replicationClusterId), sourceBaseNamespaceDirPath, sourceHFileArchiveDirPath, bulkLoadHFileMap, conf, getConnection(), entry.getKey());
                        hFileReplicator.replicate();
                        if (LOG.isDebugEnabled()) {
                            LOG.debug("Finished replicating bulk loaded data from cluster id: {}", entry.getKey().toString());
                        }
                    }
                }
            }
