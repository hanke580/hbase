 ...
 */
@edu.umd.cs.findbugs.annotations.SuppressWarnings(value = "VO_VOLATILE_INCREMENT", justification = "False positive according to http://sourceforge.net/p/findbugs/bugs/1032/")
...
abstract class ServerRpcConnection implements Closeable {
    ...
    private String getFatalConnectionString(final int version, final byte authByte) {
        return "serverVersion=" + RpcServer.CURRENT_VERSION + ", clientVersion=" + version + ", authMethod=" + authByte + ", authSupported=" + (authMethod != null) + " from " + toString();
    }
     ...
     * @throws FatalConnectionException
     */
    private void setupCryptoCipher(final ConnectionHeader header, RPCProtos.ConnectionHeaderResponse.Builder chrBuilder) throws FatalConnectionException {
        ...
        properties.setProperty(CryptoCipherFactory.CLASSES_KEY, this.rpcServer.conf.get("hbase.rpc.crypto.encryption.aes.cipher.class", "org.apache.commons.crypto.cipher.JceCipher"));
    ...
    public void saslReadAndProcess(ByteBuff saslToken) throws IOException, InterruptedException {
        if (saslContextEstablished) {
            RpcServer.LOG.trace("Read input token of size={} for processing by saslServer.unwrap()", saslToken.limit());
            if (!useWrap) {
            ...
            try {
                if (saslServer == null) {
                    saslServer = new HBaseSaslRpcServer(authMethod, rpcServer.saslProps, rpcServer.secretManager);
                    RpcServer.LOG.debug("Created SASL server with mechanism={}", authMethod.getMechanismName());
                }
                RpcServer.LOG.debug("Read input token of size={} for processing by saslServer." + "evaluateResponse()", saslToken.limit());
                replyToken = saslServer.evaluateResponse(saslToken.hasArray() ? saslToken.array() : saslToken.toBytes());
            } catch (IOException e) {
                ...
                RpcServer.AUDITLOG.warn(RpcServer.AUTH_FAILED_FOR + clientIP + ":" + saslServer.getAttemptingUser());
                throw e;
            }
            if (replyToken != null) {
            ...
            if (saslServer.isComplete()) {
                ...
                if (RpcServer.LOG.isDebugEnabled()) {
                    RpcServer.LOG.debug("SASL server context established. Authenticated client: " + ugi + ". Negotiated QoP is " + qop);
                }
    ...
    private void processConnectionHeader(ByteBuff buf) throws IOException {
        if (buf.hasArray()) {
            ...
            org.zlab.ocov.tracker.Runtime.update(this, 118, buf);
        } else {
            ...
            org.zlab.ocov.tracker.Runtime.update(this, 119, buf);
        }
    ...
    protected final RpcResponse getErrorResponse(String msg, Exception e) throws IOException {
        ...
        ByteBuffer headerBuf = ServerCall.createHeaderAndMessageBytes(null, headerBuilder.build(), 0, null);
    ...
    protected final boolean processPreamble(ByteBuffer preambleBuffer) throws IOException {
        ...
        for (int i = 0; i < RPC_HEADER.length; i++) {
            if (RPC_HEADER[i] != preambleBuffer.get()) {
                doBadPreambleHandling("Expected HEADER=" + Bytes.toStringBinary(RPC_HEADER) + " but received HEADER=" + Bytes.toStringBinary(preambleBuffer.array(), 0, RPC_HEADER.length) + " from " + toString());
                return false;
            }
        }
    ...
    public abstract ServerCall<?> createCall(int id, BlockingService service, MethodDescriptor md, RequestHeader header, Message param, CellScanner cellScanner, long size, InetAddress remoteAddress, int timeout, CallCleanup reqCleanup);
